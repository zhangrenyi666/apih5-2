                                        ...
                                        return (
                                            <div style={{ textAlign: 'start', padding: '10px' }}><Button type="primary" onClick={() => {
                                                ...
                                                <Button type="primary" style={{ marginLeft: '10px' }} onClick={() => {
                                                    let value = this.formHasTicket.form.getFieldsValue();
                                                    var URL = `${ureport}excel?_u=file:ZxSkResInOutStockAllAmt.xml&url=${domain}&delFlag=0&companyId=${value.companyId ? value.companyId : null}&useEndDate=${value.useEndDate ? value.useEndDate : null}&eduLevel=${value.eduLevel ? value.eduLevel : null}&title=${value.title ? value.title : null}&isWorking=${value.isWorking ? value.isWorking : null}`;
                                                    if (value.companyId) {
                                                        window.open(URL);
                                                    } else {
                                                        Msg.warn('请选择单位名称！')
