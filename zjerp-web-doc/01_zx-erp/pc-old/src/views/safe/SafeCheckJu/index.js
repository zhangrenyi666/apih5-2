...
class index extends Component {
  ...
  render() {
    const { companyId, companyName, departmentId } = this.props.loginAndLogoutInfo.loginInfo.userInfo.curCompany;
    return (
      <div>
        <QnnTable
          {...this.props}
          ...
          {...config}
          fetchConfig={{
            apiName: 'getZxSfCheckListAllBureau',
            otherParams: {
              checkType: '1',
          ...
          actionBtns={[
            ...
            },
            {
              ...
              field: 'editBtn',
              onClick: (obj) => {
                if (obj.selectedRows[0].isReported === '2' || obj.selectedRows[0].isSend === '1') {
                  Msg.warn('局下达、公司上报的数据不能修改！');
                  this.table.closeDrawer();
                } else {
                  if (obj.selectedRows[0].checkAgainStatus) {//局自己建立的数据-复查后不可以修改
                    Msg.warn('请选择未复查的数据！');
                    this.table.closeDrawer();
                  } else {
            ...
            },
            {
              ...
              onClick: (obj) => {
                ...
                for (let m = 0; m < obj.selectedRows.length; m++) {
                  if (obj.selectedRows[m].isSend === '1' || obj.selectedRows[m].checkAgainStatus || obj.selectedRows[m].isReported === '2') {
                    //存在局下达、公司上报、复查后的数据
            ...
            },
            {
              ...
              formBtns: [
                ...
                },
                {
                  ...
                  label: '保存',
                  fetchConfig: {
                    apiName: 'updateZxSfCheckAgainStatusBureau'
                  }
                }
              ]
            },
            {
              ...
              onClick: (obj) => {
                ...
                if (obj.selectedRows.length === 1) {
                  // if (obj.selectedRows[0].checkAgainStatus ==='1') {
                    confirm({
                      content: '确定下达选中的数据吗?',
                      onOk: () => {
                        myFetch('updateZxSfCheckIsSendBureau', obj.selectedRows[0]).then(
                          ({ data, success, message }) => {
                            if (success) {
                              Msg.success(message)
                              this.table.refresh();
          ...
          formConfig={[
            ...
            },
            {
              ...
              form: {
                ...
                fetchConfig: {
                  apiName: 'getZxCtContractListByCompanyId',
                  params: {
                    companyId: 'companyId'
                  },
                },
