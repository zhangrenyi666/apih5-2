...
class index extends Component {
    constructor(props) {
        ...
        this.state = {
            loading: false,
            dataTow: [{ value: 85, name: '2年以下' }, { value: 85, name: '2-5年' }, { value: 85, name: '5-10年' }, { value: 85, name: '10年以上' }],
            dataSource: [],
            startDate: undefined,
            endDate: undefined
        }
            ...
            if (ev) {
                switch (row.type) {
                    ...
                    case 'company':
                        ...
                        break
                    case 0:
                        this.fourTypesGetNextNodeContentSetTreeFunction('GD', 'getZxSfProEmpMainWorkAgeGuiDangList', row)
                        break
                    case 1:
                        this.fourTypesGetNextNodeContentSetTreeFunction('JG', 'getZxSfProEmpMainWorkAgeJiaoGongList', row)
                        break
                    case 2:
                        this.fourTypesGetNextNodeContentSetTreeFunction('WG', 'getZxSfProEmpMainWorkAgeWanGongList', row)
                        break
                    case 3:
                        this.fourTypesGetNextNodeContentSetTreeFunction('KG', 'getZxSfProEmpMainWorkAgeKaiGongList', row)
                        break
                }
            }
        }
            ...
            if (res.data) {
                ...
                statusDataList = res.data.map((item, index) => {
                    return {
                        ...
                        less2: res.data.less2,
                        '2to5': res.data['2to5'],
                        '5to10': res.data['5to10'],
                        more10: res.data.more10,
    ...
    setBureauLevelSecondNodeFunc(row) {
        this.props.myFetch('getZxSfProEmpMainWorkAgeComList', {
            startDate: this.state.startDate,
            endDate: this.state.endDate,
            companyId: this.props.loginAndLogoutInfo.loginInfo.userInfo.curCompany.companyId
        }).then(res => {
    ...
    companyToProjectTypeSetNodeFunction(row) {
        this.props.myFetch('getZxSfProEmpMainpWorkAgeOrgList', {
            startDate: this.state.startDate,
            endDate: this.state.endDate,
            companyId: this.props.loginAndLogoutInfo.loginInfo.userInfo.curCompany.companyId
        }).then(res => {
    ...
    initializationRootNodeFunction() {
        switch (this.props.loginAndLogoutInfo.loginInfo.userInfo.ext1) {
            // 局级单位
            case '1':
                this.setRootNodeFunc('getZxSfProEmpMainWorkAgJuInfo', 'bureauLevel')
                break
            // 公司级单位
            case '2':
                this.setRootNodeFunc('getZxSfProEmpMainWorkAgeComInfo', 'company')
                break
        }
    }
    ...
    setRootNodeFunc(apiName, type) {
        this.props.myFetch(apiName, {
            ...
            if (res.data) {
                ...
                // 饼状图的对应数据list
                const pieChartData = [res.data.less2, res.data['2to5'],res.data['5to10'], res.data.more10]
