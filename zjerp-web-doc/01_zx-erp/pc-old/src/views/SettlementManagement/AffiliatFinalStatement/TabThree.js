            ...
            },
            {
              ...
              qnnTableConfig: {
                fetchConfig: () => {
                  if (params.tabIndex === '2') {
                    return {
                      ...
                      otherParams: { zxSaFsPaySettlementId: this.table?.form.getFieldValue('zxSaFsPaySettlementId') }
                    }
                  } else {
                    return {}
                  }
                ...
                rowDisabledCondition: (rowData) => {
                  return rowData.zxSaFsPaySettlementDetailId !== this.table.form.getFieldValue('editLineName')
                },
                paginationConfig: {
                  position: 'bottom'
                },
                    ...
                    onClick: (obj) => {
                      ...
                      if (tableOneData && tableOneData.length > 0) {
                        for (var i = 0; i < tableOneData.length; i++) {
                          if (tableOneData[i].zxSaFsPaySettlementDetailId === this.table.form.getFieldValue('editLineName')) {
                      ...
                      let newRowData = {}
                      newRowData.zxSaFsPaySettlementDetailId = new Date().getTime().toString()
                      tableOneData.push(newRowData)
                      this.tableOne.btnCallbackFn.setState({
                        data: tableOneData
                      })
                      this.table.form.setFieldsValue({
                        editLineName: newRowData.zxSaFsPaySettlementDetailId
                      })
                    },
                  },
                  {
                      ...
                      confirm({
                        ...
                        onOk: () => {
                          ...
                          for (var i = 0; i < data.length; i++) {
                            if (data[i].zxSaFsPaySettlementDetailId === this.table.form.getFieldValue('editLineName')) {
                              Msg.warn('有未保存数据，请保存后重试。')
                              return
                            }
                          }
                          let deleteData = obj.selectedRows
                          this.props.myFetch('batchDeleteUpdateZxSaFsPaySettlementDetail', deleteData).then(
                            ({ success, message }) => {
                              if (success) {
                                Msg.success(message);
                ...
                formConfig: [
                    ...
                    form: {
                      ...
                      onChange: (val, obj) => {
                        let tableOneData = this.tableOne.state.data.map((rowData) => {
                          if (rowData.zxSaFsPaySettlementDetailId === this.table.form.getFieldValue('editLineName')) {
                      ...
                      onBlur: (obj) => {
                        ...
                        let tableOneData = this.tableOne.state.data.map((rowData) => {
                          if (rowData.zxSaFsPaySettlementDetailId === this.table.form.getFieldValue('editLineName')) {
                            ...
                            rowData.thisAmt = (rowData.price ? rowData.price : 0) * val  //本期结算金额
                            if (rowData.taxRate && rowData.taxRate !== '0') {
                              rowData.thisAmtNoTax = ((rowData.price ? rowData.price : 0) * val) / (1 + Number(rowData.taxRate) / 100)  //本期结算不含税金额
                              rowData.thisAmtTax = (rowData.price ? rowData.price : 0) * val - ((rowData.price ? rowData.price : 0) * val) / (1 + Number(rowData.taxRate) / 100)//本期结算税额
                            }
                          }
                          return rowData;
                        })
                  ...
                  },
                  {
                    ...
                    form: {
                      ...
                      onBlur: (obj) => {
                        ...
                        let tableOneData = this.tableOne.state.data.map((rowData) => {
                          if (rowData.zxSaFsPaySettlementDetailId === this.table.form.getFieldValue('editLineName')) {
                            rowData.price = Number(val)
                            rowData.thisAmt = (rowData.qty ? rowData.qty : 0) * val
                            if (rowData.taxRate && rowData.taxRate !== '0') {
                  ...
                  },
                  {
                    ...
                    table: {
                      ...
                      onClick: (obj) => {
                        if (obj.rowData.zxSaFsPaySettlementDetailId === this.table.form.getFieldValue('editLineName')) {
                          Toast.loading('loading...');
                          setTimeout(() => {
                            let body = {
                              ...obj.rowData
                            };
                            body.zxSaFsSettlementId = params.zxSaFsSettlementId
                            body.zxSaFsPaySettlementId = this.table.form.getFieldValue('zxSaFsPaySettlementId');
                            this.props.myFetch(this.isNumber(obj.rowData.zxSaFsPaySettlementDetailId) ? 'addZxSaFsPaySettlementDetail' : 'updateZxSaFsPaySettlementDetail', body).then(
                              ({ success, message }) => {
                                if (success) {
                                  Msg.success(message);
                                  this.table.form.setFieldsValue({
                          ...
                          this.table.form.setFieldsValue({
                            editLineName: obj.rowData.zxSaFsPaySettlementDetailId
                          })
                          obj.btnCallbackFn.refresh();
                        }
                      }
