...
class index extends Component {
  ...
  async Itemcompute() {
    ...
    newRowData.mcsgPriceTax = rowData.mcsgPrice && rowData.taxRate
      ? (this.FloatMulTwo(rowData.mcsgPrice, Number(rowData.taxRate)) / 100).toFixed(2) : 0;
    //运费税金单价(ysFeeTax) => 运费(不含税)(ysFee) * 运费税率(ysTaxRate)
    newRowData.ysFeeTax = rowData.ysFee && rowData.ysTaxRate
      ? (this.FloatMulTwo(rowData.ysFee, Number(rowData.ysTaxRate)) / 100).toFixed(2) : 0;
    //不含税到场价(元)(scenePrice) => 材料价格(不含税)(mcsgPrice) + 运费(不含税)(ysFee)
    newRowData.scenePrice = rowData.mcsgPrice && rowData.ysFee
      ? (Number(rowData.mcsgPrice) + Number(rowData.ysFee)).toFixed(2) : 0;
    ...
    newRowData.taxAmt = rowData.qty
      ? (this.FloatMulTwo(rowData.qty, price)).toFixed(2) : 0
