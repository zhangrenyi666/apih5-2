...
export default memo((props) => {
    ...
    const FlowFormComFC = (obj) => {
        ...
        useEffect(() => {
            (async () => {
                const { success, message, code, data } = await props.myFetch("bxCheckZxGcsgCtContrApplyBeforeFlow", {
                    ctContrApplyId: obj.selectedRows[0]?.ctContrApplyId
                });
        ...
        if (checkIng) {
            return <div style={{ padding: 12, textAlign: "center" }}>
                <a><LoadingOutlined /> loading...</a>
            </div>
        }

        return <FlowForm {...props} btnCallbackFn={obj.btnCallbackFn} isInQnnTable={obj.isInQnnTable} flowFormData={obj.selectedRows[0]} />
    ...
    const willOpenFlowFormCom = useCallback((obj) => {
        ...
        if (obj.selectedRows[0].workId && obj.selectedRows[0].workId !== "") {
    ...
    const changeJS = async (event, formBtnCallbackFn, tableBtnCallbackFn, params = []) => {
        // console.log('执行changeJS')
        //---修改中   ing...
        //以前不使用tableTdEdit模式 所以使用操作表格数据进行逻辑编写
        //不使用tableTdEdit模式是因为某些单元格需要根据一些规则动态禁用。还有类型也会改变（---需要新增支持）


        // const { getEditedRowData, setEditedRowData } = tableBtnCallbackFn;
        // const curEditRow = await getEditedRowData(false);
        ...
        // const taxRate = formBtnCallbackFn.itemData?.['value'] || curEditRow['taxRate'];
        // const newAttr = {
        //     //先备份 变更后数量	因为这个字段需要不断根据一个值变化
        //     //有备份字段优先使用备份字段
        //     _afterChangeQty: curEditRow["_afterChangeQty"] || curEditRow["afterChangeQty"]
        // };
        // if (curEditRow.alterType === "1") {
        //     //新增 
        ...
        //         //这里只能用备份的字段来操作
        //         newAttr["afterChangeQty"] = Number(newAttr['_afterChangeQty']) + Number(curEditRow['changeQty']);
        //     }

        //     newAttr["afterAmt"] = newAttr["afterChangeQty"] * curEditRow['afterChangePrice'];
        ...
        //     ...newAttr
        // }
        // setEditedRowData(newRowData)


        //获取正在编辑的行数据 然后修改整个表格数据  然后设置值进form
        const curEditRow = formBtnCallbackFn.record || formBtnCallbackFn.funcCallBackParams?.rowData || {};
        ...
        const { ccCoAlterWorkId, alterType } = curEditRow;
        //税率字段特殊 因为下拉需要从 itemData(只有下拉类型有 itemData) 中取
        const taxRate = Number(formBtnCallbackFn.itemData?.['value']) || Number(curEditRow['taxRate']) || 0;
        // console.log("curEditRow", curEditRow)
        //需要手动改变表格数据   
        const newTableData = loopTableData(alterWorkList, (item) => {
            const newAttr = {};
            if (item["ccCoAlterWorkId"] === ccCoAlterWorkId) {
                if (alterType === "1") {
                    //新增 
                    newAttr["afterChangeQty"] = curEditRow['changeQty'];
                    newAttr["afterChangePrice"] = curEditRow['changePrice'];
                    ...
                    if (curEditRow['changeQty']) {
                        //这里只能用备份的字段来操作
                        newAttr["afterChangeQty"] = Number(item['quantity']) + Number(curEditRow['changeQty']);
                    }
        ...
        // console.log("设置值：", newTableData) 
        qingdanRef.current.setValues({
            alterWorkList: newTableData
        })
        //计算完表格数据后才可以计算表单
        setTimeout(() => {
            tableDatachangeJS()
    ...
    const tableDatachangeJS = () => {
        ...
        const {
            alterWorkList = [],
        } = qingdanRef.current.form.getFieldsValue();


        let applyAmount = 0.000000;
        let applyAmountNoTax = 0.000000;
        let applyAmountTax = 0.000000;
    ...
    const QingDanEdit = (tableProps) => {
        ...
        // const field = tableProps.field;
        // if (field === "xiMingBianji") {
        //     formConfig = formConfig.filter(item => item.field === 'alterWorkList');
        //     qingDanFormConfig.tailFormItemLayout = {
        //         wrapperCol: {
        //             sm: { offset: 0 }
        //         }
        //     }
    ...
    const GuaJia = (tableProps) => {
        return <div style={{ padding: 12 }}>
            <QnnForm
                ...
                upload={props.myUpload}
                method={{
                    getTabThreeTableOtherParams, tabThreeDataFormat,
                    getTabFourTableOtherParams, synchronouslySuspendData
                }}
                rowData={tableProps.rowData}
    ...
    return <>
        <QnnTable
            ...
            componentsKey={{ FlowFormCom: FlowFormComFC, QingDanEdit, GuaJia }}
            method={{
                qsjdcx: qsjdcx,
                firstIDChange: firstIDChange, contractNameChange,
                willOpenFlowFormCom,
