            ...
            table: {
                ...
                fieldConfig: {
                    onChange: async (colVal, formBtnCallbackFn, tableBtnCallbackFn) => {
                        ...
                        //获取到行id 然后进行数据更改
                        // 获取到下拉选项数据
                        const { ccCoAlterWorkId } = formBtnCallbackFn.funcCallBackParams?.rowData || {};

                        //获取表单对象
                        const qnnFormRef = tableBtnCallbackFn.props.qnnFormProps.qnnformData.qnnFormProps.getQingdanRef();

                        const formVals = await qnnFormRef.getValues(false);
                        //获取到变更细明表格数据 然后修改后重新赋值
                        let { alterWorkList = [] } = formVals;
                        alterWorkList = alterWorkList.map(item => {
                        ...
                        //这里需要延时设置
                        setTimeout(() => {
                            qnnFormRef.setValues({
                                ...formVals,
                                alterWorkList: alterWorkList
                            });
                        }, 10)
            ...
            table: {
                ...
                fieldConfig: {
                    ...
                    onChange: async (colVal, formBtnCallbackFn, tableBtnCallbackFn) => {
                        ...
                        // 获取到下拉选项数据
                        const { ccCoAlterWorkId } = formBtnCallbackFn.funcCallBackParams?.rowData || {};
                        const itemData = formBtnCallbackFn.itemData[0] || {};
            ...
            table: {
                ...
                fieldConfig: {
                    ...
                    onBlur: async (colVal, formBtnCallbackFn, tableBtnCallbackFn) => {
                        const { alterType, ccCoAlterWorkId } = formBtnCallbackFn?.record || {};
                        //获取表单对象 
                        if (alterType === "1") {
                            const qnnFormRef = tableBtnCallbackFn.props.qnnFormProps.qnnformData.qnnFormProps.getQingdanRef();
                            const formVals = await qnnFormRef.getValues(false);
