...
export default memo((props) => {
    ...
    const packageNoChange = useCallback(async (val, args) => {
        const { itemData } = args;
        //已经不让选择了
        // if(itemData[0]?.isRela && +itemData[0]?.isRela === 0){
        //     Modal.error({
        ...
        // }
        rootTable.current.qnnForm.setValues({
            cloudBidNo: itemData[0]?.schemeNo
        })
    }, [])
    ...
    const rowImportClick = useCallback((args) => { 
        ...
        if (args.rowData.workId && args.rowData.workId !== "" && args.rowData.apih5FlowStatus !== '3') {
    ...
    const willOpenFlowFormCom = useCallback((obj, params) => {
        ...
        //评审不通过的数据可以修改、清单导入 apih5FlowStatus === '3'
        if (obj.rowInfo.name === 'edit' && obj.selectedRows[0].apih5FlowStatus === '3') {
            return true;
        } 
    ...
    return <>
        <QnnTable
            ...
            method={{
                ...
                tabFourGXSaveCb, tabFourFormFieldOnBlur,

                qsjdcx: () => { console.log('签收进度') },
