...
class operation extends Component {
    ...
    componentDidMount() {
        const { title, workId } = this.props.selectedRows[0];
        const { apiName, detailApiName, flowId } = this.props;
        if (workId) {
            this.props.myFetch(apiName, { apiName: detailApiName, apiType: 'POST', flowId: flowId, title: title, workId: workId }).then(({ success, message, data }) => {
                if (success) {
                    this.setState({
        ...
        return (
            <div style={{ padding: '10px' }}>
                <div>
                    ...
                    {
                        flowWebUrl ? <div>
                            <iframe width="100%" height={ContentH * 0.5} frameBorder={0} src={flowWebUrl} title="byLm"></iframe>
                        </div> : null
                    }
                </div>
            </div>
