...
class index extends Component {
    ...
    getDjbhFun(val, busDate) {
        ...
        myFetch('getZxSkStockTransferDishedOutNo', {
            whOrgID: val,
    ...
    dateChange(val, type) {
        ...
        if (type === 1) {
            ...
            if (whOrgID) {
                this.getDjbhFun(whOrgID, busDate);
            }
        }
    }
    handleCancel = () => {
    ...
    render() {
        ...
        const { ext1, departmentId, projectId, companyName, companyId, projectName } = this.props.loginAndLogoutInfo.loginInfo.userInfo.curCompany;
        ...
        return (
            <div>
                <QnnTable
                    ...
                    {...config}
                    fetchConfig={{
                        apiName: 'getZxSkStockTransferDishedOutList',
                        otherParams: {
                    ...
                    formConfig={[
                            ...
                            form: {
                                ...
                                onChange: (val) => {
                                    if (this.table && this.table.qnnForm) {
                                        this.table.qnnForm.clearValues(['zxSkStockTransItemDishedOutList']);
                                        let busDate = this.table.qnnForm.form.getFieldsValue().busDate;
                                        if (busDate || val) {
...

                        {
                            ...
                            form: {
                                type: 'qnnTable',
                                field: 'zxSkStockTransItemDishedOutList',
                                incToForm: true,
                                qnnTableConfig: {
                                    antd: {
                                        rowKey: 'id',
                                                ...
                                                onChange: async (colVal, thisProps, tableBtnCallbackFn) => {
                                                    if (colVal >= 0 && typeof (colVal) === 'number') {
                                                        ...
                                                        if (rowData.outQty > rowData.stockQty) {
                                                            ...
                                                            newRowData.outQty = Number(rowData.outQty.toFixed(3));
                                                        }
                                            ...
                                            form: {
                                                ...
                                                onChange: async (colVal, thisProps, tableBtnCallbackFn) => {
                                                    if (colVal >= 0 && typeof (colVal) === 'number') {
                                                        ...
                                                        newRowData.outFee = Number(rowData.outFee.toFixed(2));
                                                        await tableBtnCallbackFn.setEditedRowData({
                                                            ...newRowData
                                                        });
                                                    }
                                            ...
                                            form: {
                                                ...
                                                onChange: async (colVal, thisProps, tableBtnCallbackFn) => {
                                                    if (colVal >= 0 && typeof (colVal) === 'number') {
                                                        ...
                                                        newRowData.outAmt = Number(rowData.outAmt.toFixed(2));
                                                        await tableBtnCallbackFn.setEditedRowData({
                                                            ...newRowData
                                                        });
                                                    }
                            ...
                            if (obj.selectedRows.length === 1) {
                                if (obj.selectedRows[0].billStatus === '1') {
                                    ...
                                    confirm({
                                        content: '确定审核选中的数据吗?',
                                        onOk: () => {
                                            obj.selectedRows[0].outOrgID = jurisdiction;

                                            myFetch('checkZxSkStockTransferDishedOut', { id: obj.selectedRows[0].id }).then(
                                                ({ data, success, message }) => {
                                                    if (success) {
                                                        Msg.success(message);
                                                        this.table.refresh();
                        ...
                        onClickFunDel: (obj) => {
                            ...
                            if (arry.length > 0) {
                                ...
                                confirm({
                                    content: '确定删除选中的数据吗?',
                                    onOk: () => {
                                        myFetch('batchDeleteUpdateZxSkStockTransferDishedOut', obj.selectedRows).then(
                                            ({ data, success, message }) => {
                                                if (success) {
                                                    this.table.refresh();
                                                } else {
                    ...
                    //                 field: 'addsubmit',
                    //                 fetchConfig: {
                    //                     apiName: 'addZxSkStockTransferDishedOut'
                    //                 }
                    //             }
                    //         ]
                    //     },
                    ...
                    //                 label: '保存',
                    //                 fetchConfig: {
                    //                     apiName: 'updateZxSkStockTransferDishedOut'
                    //                 }
                    //             }
                    //         ]
                    //     },
                ...
                >
                    {
                        idPreview ? <Spin spinning={loading}>
                            ...
                            }} src={`${ureport}preview?_u=minio:ZxSkStockTransferDishedOut.preview.xml&_t=1,6&_n=${name}&access_token=${access_token}&id=${idPreview}
                            `}title="byLm"></iframe>
                        </Spin> : ''
                    }
                            ...
                            btns={[
                                ...
                                },
                                {
                                    ...
                                    onClick: (obj) => {
                                        ...
                                        this.props.myFetch('updateZxSkStockTransferDishedOut', values).then(
                                            ({ success, message }) => {
                                                if (success) {
                                                    Msg.success(message);
                                                    this.table.refresh();
