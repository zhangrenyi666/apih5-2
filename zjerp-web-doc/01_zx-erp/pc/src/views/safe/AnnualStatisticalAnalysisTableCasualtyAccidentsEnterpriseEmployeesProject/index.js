                                        ...
                                        return (
                                            <div style={{ textAlign: 'start', padding: '10px' }}><Button type="primary" onClick={() => {
                                                ...
                                                <Button type="primary" style={{ marginLeft: '10px' }} onClick={() => {
                                                    ...
                                                    console.log(value)
                                                    var URL = `${ureport}excel?_u=minio:ZxSfAccidentItemYear.ureport.xml&access_token=${access_token}&_n=企业职工伤亡事故年度统计分析表（项目）.xlsx&delFlag=0&orgID=${value.projectID ? value.projectID : null}&year=${value.year ? new Date().getTime(value.year) : ''}`;
                                                    if (value.projectID) {
                                                        if (value.year) {
                                                            window.open(URL);
                                                        } else {
