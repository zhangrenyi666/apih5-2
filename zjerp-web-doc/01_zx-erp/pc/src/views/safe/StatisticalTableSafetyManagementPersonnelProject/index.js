                                        ...
                                        return (
                                            <div style={{ textAlign: 'start', padding: '10px' }}><Button type="primary" onClick={() => {
                                                ...
                                                if (value.orgID) {
                                                    setTimeout(() => {
                                                        this.setState({
                                                            ...
                                                            sendCardDate: value.sendCardDate ? value.sendCardDate : null,
                                                ...
                                                <Button type="primary" style={{ marginLeft: '10px' }} onClick={() => {
                                                    ...
                                                    var URL = `${ureport}excel?_u=minio:ProjectEmployeeForm.ureport.xml&access_token=${access_token}&_n=安全管理人员统计表（项目）.xlsx&delFlag=0&orgID=${value.orgID ? value.orgID : null}&year=${value.year ? new Date().getTime(value.year) : null}&eduLevel=${value.eduLevel ? value.eduLevel : null}&title=${value.title ? value.title : null}&isWorking=${value.isWorking ? value.isWorking : null}&sendCardDate=${value.sendCardDate ? new Date().getTime(value.sendCardDate) : null}&useEndDate=${value.useEndDate ? new Date().getTime(value.useEndDate) : null}`;
                                                    if (value.orgID) {
                                                        window.open(URL);
                                                    } else {
                                                        Msg.warn('请选择单位名称！')
