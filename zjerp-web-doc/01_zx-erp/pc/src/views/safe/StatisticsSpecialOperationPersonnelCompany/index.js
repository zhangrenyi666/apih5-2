                                        ...
                                        return (
                                            <div style={{ textAlign: 'start', padding: '10px' }}><Button type="primary" onClick={() => {
                                                ...
                                                <Button type="primary" style={{ marginLeft: '10px' }} onClick={() => {
                                                    ...
                                                    var URL = `${ureport}excel?_u=minio:zxSfSpecialCom.ureport.xml&access_token=${access_token}&_n=特种作业人员统计表（公司）.xlsx&delFlag=0&companyId=${value.companyId ? value.companyId : null}&opProjName=${value.opProjName ? value.opProjName.join(',') : ''}&endDate=${value.endDate ? new Date().getTime(value.endDate) : ''}`;
                                                    if (value.companyId) {
                                                        window.open(URL);
                                                    } else {
                                                        Msg.warn('请选择单位名称！')
