            ...
            <div>
                                        ...
                                        return (
                                            <div style={{ textAlign: 'start', padding: '10px' }}><Button type="primary" onClick={() => {
                                                ...
                                                <Button type="primary" style={{ marginLeft: '10px' }} onClick={() => {
                                                    ...
                                                    console.log(value)
                                                    var URL = `${ureport}excel?_u=minio:ZxSfAccidentItemYearCom.ureport.xml&access_token=${access_token}&_n=企业职工伤亡事故年度统计分析表（公司）.xlsx&delFlag=0&companyId=${value.companyId ? value.companyId : null}&year=${value.year ?new Date().getTime(value.year) : ''}`;
                                                    if (value.companyId) {
                                                        if (value.year) {
                                                            window.open(URL);
                                                        } else {
                ...
                <QnnTable
                    ...
                    fetchConfig={companyId ? {
                        apiName: 'getZxSfAccidentItemUReportFormYearComList',
                        otherParams: {
                            companyId: companyId,
                            year: year
                        }
