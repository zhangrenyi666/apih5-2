...
import { Button, Select, message, Row, Col, Input, Spin } from 'antd';
...
const ListSubcontracts = (props) => {
    ...
    //表单主键
    const rootId = props.rowData["ctContrApplyId"];

    // 完成后的数据不让编辑
    const isOver = props.rowData?.apih5FlowStatus === "2"; 
    //首次渲染
    ...
    const myFetch = useCallback(props.fetch, [])
    ...
    //右侧视图 table（清单列表） | gjTable（挂接列表） | editLowerList（编辑下级清单列表）
    ...
    //获取工序库下拉列表
    const getGXK = async () => {
        const { success, message, code, data } = await myFetch('gcsgGetZxCtSZProcessSelect', {});
        if (success) {
            return data
        } else {
    ...
    const getGjTableData = async () => {
        const { success, message, code, data } = await myFetch('gcsgGetZxCtSZProcessList', {
    ...
    useEffect(() => {
        ...
        } else if (rightView === 'table') {
            //因为没有左侧树节点 所以首次也需要直接去请求数据 
            refreshRootTable();
        }
    }, [gxValue, rightView])
    ...
    const gjTableChangeSelect = useCallback(async (selectedRowKeys, tableSelectedRows) => {
        setGjLoading(true)
        const { success, message, code, data } = await myFetch('manualHookZxGcsgCcCoAlterWorkProcess', {
            inputWorkType: baseType,
            ccCoAlterWorkId: curGJRowData.current?.ccCoAlterWorkId,
    ...
    const getRootTableData = async () => {
        const { success, message, code, data } = await myFetch('getZxGcsgCcCoAlterWorkListAmount', {
    ...
    const refreshRootTable = async () => {
        ...
        setRightViewLoading(true)
        const listData = await getRootTableData();
        if (!mounted.current) return;
        setRightViewLoading(false)
        if (listData) {
    ...
    const jjgzSaveClick = useCallback(async () => {
        const { success, message, code } = await myFetch('manualHookZxGcsgCcCoAlterWorkRule', {
    ...
    const tableView = (<>
        ...
        <QnnTable
            ...
            formConfig={isOver ? tabTwoRootTable.formConfig.filter((item) => item.table?.dataIndex !== "action") : tabTwoRootTable.formConfig}
            curEnv={curEnv}
            antd={{
                    ...
                    <Row align="middle">
                        <Col span={19}>
                            <span style={{ width: 500, display: "inlineBlock" }}>
                                {
                                    useMemo(() => <QnnForm {...qnnTablecomProps} style={{ paddingBottom: 0 }} wrappedComponentRef={(qnnForm) => jjgzRef.current = qnnForm}>
    ...
    return <div className={`${style.container} ${curEnv === 'detail' ? style.detailContainer : ''}`}>
        <div className={`${style.tableContainer} ${style.fullWidth}`} key={rightView}>
