...
import { PullRequestOutlined, RollbackOutlined, SaveOutlined } from '@ant-design/icons';
...
const ListSubcontracts = (props) => {
    ...
    const getGjTableData = async () => {
        const { success, message, code, data } = await rootTable.fetch('gcsgGetZxCtSZProcessList', {
            ctContractId: rootId, //最外层表格的id
            ccWorksId: curGJRowData.current?.ccWorksId, //抽屉中清单id
    ...
    const gjTableChangeSelect = useCallback(async (selectedRowKeys, tableSelectedRows) => {
        setGjLoading(true)
        const { success, message, code, data } = await rootTable.fetch('manualHookZxGcsgCcWorksProcess', {
            inputWorkType: baseType,
            ccWorksId: curGJRowData.current?.ccWorksId,
    ...
    const getRootTableData = async ({ treeNode }) => {
        const { success, message, code, data } = await rootTable.fetch('getZxGcsgCcWorksListAmount', {
            treeNode,
            ctContractId: rootId
        });
        if (success) {
    ...
    const jjgzSaveClick = useCallback(async () => {
        const { success, message, code } = await rootTable.fetch('manualHookZxGcsgCcWorksRule', {
