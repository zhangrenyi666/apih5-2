...
//工序数据中单价计算
//传入行数据 @rowData
const getRowPrice = ({ rgf = 0, zzcl = 0, jxsb = 0 }) => {
   return rgf + zzcl + jxsb
}
...
export default {
   ...
   actionBtns: [
      ...
      // },
      {
         ...
         hide: (args) => args.props.clickCb?.rowInfo?.name === "detail"
      },
   ],
   formConfig: [
      {
      ...
      },
      {
         ...
         form: {
            ...
            precision: 6, //数值精度 
            onChange: "bind:tabFourFormFieldOnBlur",
         }
      },
      {
         table: {
      ...
      //工序表格
      {
         ...
         form: {
            ...
            qnnTableConfig: {
               ...
               // fetchConfig: {
               //    apiName: "ctPriceSysItemList",
               //    // otherParams: {
               //    //    itemId: "xingzhengquhuadaima"
               //    // }
               // },
               //点保存按钮后将计算上方表单中的数据
               tableTdEditSaveCb: "bind:tabFourGXSaveCb",
               formConfig: [
                  {
                     table: {
                        title: '工序编码',
                  ...
                  },
                  {
                     table: {
                        ...
                        dataIndex: 'rgf',
                        fieldConfig: {
                           onBlur: async (event, formBtnCallbackFn, tableBtnCallbackFn) => {
                              //计算单价 
                              const rowData = await tableBtnCallbackFn.getEditedRowData(false); // await  
                              await tableBtnCallbackFn.setEditedRowData({
                                 "price": getRowPrice(rowData)
                              });
                           }
                        }
                     }
...

      {
         isInForm: false,
         isInTable: ({ clickCb = {} }) => {
            const btnName = clickCb?.rowInfo?.name;
            return btnName !== "detail"
         },
         table: {
            showType: 'tile',
            width: 80,
