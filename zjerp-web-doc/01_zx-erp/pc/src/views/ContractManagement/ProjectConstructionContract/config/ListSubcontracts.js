...
//分包合同清单
import React, { useCallback, useRef, useState, useEffect, useMemo } from "react" //, Suspense, lazy 
...
import { Button, Select, message, Row, Col, Input, Spin, Modal, Skeleton } from 'antd';
import { PullRequestOutlined, RollbackOutlined, FormOutlined, SaveOutlined, ColumnHeightOutlined, ImportOutlined } from '@ant-design/icons';
...
// const QnnForm = lazy(() => new Promise((resolve) => {
//     setTimeout(()=>{
//         resolve(import("qnn-form"))
//     }, 1000)
// }));
const { Option } = Select;
...
const ListSubcontracts = (props) => {
    ...
    const rootId = rootTable.getQnnForm().form.getFieldValue("ctContrApplyId");
    ...
    //请求工序库loading状态
    const [gxkLoading, setGxkLoading] = useState(false);
    //请求挂接节点loading状态
    const [gjDataLoading, setGjDataLoading] = useState(false);
    ...
    //设置工序的下拉数据和下拉值
    const setGxOptionDataAndValue = (_gxkValue = gxkValue, _gxkOptionData = gxkOptionData, _gxValue) => {
        const curGXKOptionItem = _gxkOptionData?.find?.(item => item[gxkKey.value] === _gxkValue) || [];
        if (curGXKOptionItem) {
            if (!_gxValue) {
                //设置工序列表
                const gxOptionData = curGXKOptionItem[gxkKey.children] || [];
                ...
                //设置工序列表
                const gxOptionData = curGXKOptionItem[gxkKey.children] || [];
                const curGxOptionData = gxOptionData.find(item => item[gxkKey.value] === _gxkValue);
                setGxOptionData(gxOptionData);
                //设置工序选中数据 
                setGxValue(_gxValue);
                setTreeNode(curGxOptionData?.["treeNode"])
            }
    ...
    //获取 挂接列表 上面已经选择的工序库id
    const getGjTableTopId = async () => {
        const { success, message, code, data } = await rootTable.fetch('gcsgGetZxCtSZProcessSelectedOne', {
            applyAlterWorksId: curGJRowData.current?.ctContrApplyWorksId,
        });
    ...
    const showRootTableDo = () => {
        // 应让表格选中数据
        // 还需要展开所有子节点 
        // console.log(getTreeAllId(rootTableData.current, rootTableKey))
    ...
    //行数据右侧 挂接按钮
    const rowGJClick = useCallback(async (args) => {
        ...
        // 需要先请求工序库和工序的id
        const topIds = await getGjTableTopId();
        curGJRowTopIds.current = topIds;
        if (topIds.id1) {
            setGxkValue(topIds.id1)
            setGxOptionDataAndValue(topIds.id1, gxkOptionDataRef.current, topIds.id2)
        }
        if (topIds.id2) {
            setGxValue(topIds.id2)
        }
    ...
    useEffect(() => {
        if (rightView === 'gjTable') {
            ...
            if (treeRef.current !== "table") {
                // 需要确保不是一直在table视图 
                refreshRootTable();
                treeRef.current.onLoadData(curSelectedTreeNodeDataRef.current, true, true)
            }
    ...
    //     setGjLoading(true)
    //     const { success, message, code } = await rootTable.fetch('manualHookZxGcsgCtContrApplyWorksProcess', {
    //         inputWorkType: baseType,
    ...
    const tabTwoEditTabletableTdEditSaveCb = useCallback(async (newRowData) => {
        ...
        if (success) {
            ...
            tabTwoEditFormByTableRef.current.setRowEditByRowId(newRowData.ctContrApplyWorksId, true)
        }
        setTimeout(() => {
    ...
    //批量编辑下级清单监听change事件 
    const tabTwoEditTableByBatchtableTdEditCb = useCallback((val, arg) => { 
        const {
            rowIndex,
            colConfig: {
                dataIndex,
            },
            qnnTableInstance: { getTableData, setEditedRowData }
        } = arg;
    ...
    //批量编辑中表格字段的删除按钮
    const tabTwoEditTableByBatchDelRow = useCallback(async ({ rowData, qnnTableInstance: { setTableData }, props: { antd: { rowKey } } }) => {

        const treeNode = curSelectedTreeNodeDataRef.current?.props?.dataRef?.treeNode;
        // let rowIndex = 0;
        const deledData = [...batchEditEdData.current].filter((item, index) => {
        ...
        antdMsg.loading('loading...', 0)
        const { success, message, code } = await rootTable.fetch('batchEditAllListZxGcsgCtContrApplyWorks', {
            ctContrApplyWorksList: deledData,
            treeNode: treeNode,
            ctContrApplyId: rootId
        });
        antdMsg.destroy();
        if (success) {
            msg.successMsg(message);
            batchEditEdData.current = [...deledData];
            setTableData([...deledData])
        } else {
            msg.errMsg(message, code)
        }
    }, [])
    ...
    const tableView = rightView === "table" ? (<>
        <div style={{ marginBottom: 12, textAlign: "right", display: isDetail ? "none" : "" }}>
            ...
            <Button type="primary" style={{ marginLeft: 6 }}
                icon={<ImportOutlined key={"rowImportClick"} />}
                onClick={() => {
                    props.method.rowImportClick && props.method.rowImportClick({
                        rowData: props.parentTableInfo?.rowData,
                        importSuccessCb: () => {
                            // refreshRootTable(); 
    ...
    const GJTableView = rightView === "gjTable" ? () => {
        ...
        useEffect(() => {
            // 选择的节点的所有子节点也需要放入到选中节点中（否则就不能放入父节点，而是放入选择的子节点） 
            const selectedRows = getGJSelectedData(gjTableData);
            tabTwoGjTableRef.current.setSelectedRows(selectedRows)
            tabTwoGjTableRef.current.setExpandedRowsKey(getTreeAllId(gjTableData, gjTableKey, 999));

            //滚动到第一个选择的节点
            if (selectedRows.length) {
                setTimeout(() => {
                    tabTwoGjTableRef.current.scrollToRowById(selectedRows[0]["id"], "center")
                }, 300)
            }
        }, [])
        ...
        return ((canRender || gjDataLoading) ? <>
                        ...
                        <Row align="middle">
                            <Col span={19}>
                                <span style={{ width: 500, display: "inlineBlock" }}>
                                    {
                                        useMemo(() => <QnnForm {...qnnTablecomProps} style={{ paddingBottom: 0 }} wrappedComponentRef={(qnnForm) => {
                                            if (qnnForm) {
                                                jjgzRef.current = qnnForm
                                            }
                                        }}>
    ...
    return <div className={`${style.container} ${curEnv === 'detail' ? style.detailContainer : ''}`} onMouseUp={hrlineOnMouseUp} onMouseMove={hrlineOnMouseMove}>
        ...
        <div className={`${style.tableContainer} ${rightView !== 'table' ? style.fullWidth : ""}`} key={rightView}>
            <Spin spinning={rightViewLoading || gxkLoading}>
                {realViews}
            </Spin>
        </div>
    </div>
