      ...
      formConfig: [
         {
            ...
            qnnTableConfig: {
               ...
               fetchConfig: {
                  ...
                  // success: ({ response, qnnTableInstance })=>{
                  //    if(response.data && response.data.length === 1){ 
                  //       const rowData = response.data[0];
                  //       qnnTableInstance.setRowEditByRowId(rowData.ctContrApplyWorksId, true)
                  //    }
                  // },
               },
               ...
               actionBtns: [
                  ...
                  },
                  {
                     ...
                     willExecute: (obj) => {
                        const selectedRows = obj.qnnTableInstance.getSelectedRows();
                        let noLeaf = true;
                        const workNos = [];
                        selectedRows.forEach(({ isLeaf, workNo }) => { 
                           if (isLeaf !== 1) { 
                              noLeaf = false;
                              workNos.push(workNo)
                           }
                        ...
                        if(!noLeaf){
                           obj.qnnTableInstance.tool.message.error(`非叶子节点不可删除，清单编码：${workNos.join('、')}`)
                        }
                        return noLeaf;
                     },
                     // onClick: async (args) => {
