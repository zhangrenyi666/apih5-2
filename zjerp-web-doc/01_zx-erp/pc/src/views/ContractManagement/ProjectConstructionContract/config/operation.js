...
class operation extends Component {
    ...
    componentDidMount() {
        const { title, workId } = this.props.qnnTableInstance.getSelectedRows()[0];
        const { apiName, flowInfo, fetch } = this.props;
        if (workId) {
            fetch(apiName, { title: title, workId: workId, ...flowInfo }).then(({ success, message, data }) => {
                if (success) {
                    this.setState({
                    ...
                    </div> : null}
                    {
                        flowWebUrl ? <div>
                            <iframe
                                width="100%"
                                title="navigation"
                                height={ContentH * 0.5} frameBorder={0}
                                src={flowWebUrl} 
                            ></iframe>
                        </div> : null
                    }
