            ...
            <div>
                                                ...
                                                <Button type="primary" style={{ marginLeft: '10px' }} onClick={() => {
                                                    confirm({
                                                        ...
                                                        onOk: () => {
                                                            let value = this.formHasTicket.form.getFieldsValue();
                                                            let filter = []
                                                            value.lastLevel && filter.push('&lastLevel=' + value.lastLevel)
                                                            value.catID && filter.push('&catID=' + value.catID)
                                                            value.customerId && filter.push('&customerId=' + value.customerId)
                                                            value.organaziID && filter.push('&organaziID=' + value.organaziID)
                                                            value.period && filter.push('&period=' + moment(value.period).valueOf())
                                                            var URL = `${ureport}excel?_u=minio:ZxCrABCJYearCreditEvaItemReports.ureport.xml&access_token=${access_token}${filter.join('')}&_n=局ABC名录`;
                                                            window.open(URL);
                                                        }
                                                    })
                                                }}>导出</Button>
                ...
                {isNeedClassifyData ?
                    <QnnTable
                        ...
                        {...config}
                        fetchConfig={{
                            apiName: 'getABCZxCrJYearCreditEvaItemReports',
                            otherParams: () => {
                                let selectData = this.formHasTicket?.form?.getFieldsValue()
                                return {
                                    catID: selectData?.catID,
                                    lastLevel: selectData?.lastLevel,
                                    organaziID: selectData?.organaziID,
                ...
                >
                    {
                        <Spin spinning={loading}>
                            ...
                            }} src={`${ureport}preview?_u=minio:ZxCrABCJYearCreditEvaItemReports.ureport.xml&_t=1,6&access_token=${access_token}${this.state.filter.join('')}
                            `}></iframe>
                        </Spin>
                    }
