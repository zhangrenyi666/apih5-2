...
const onLoadData = function (treeNodeData) {
    return new Promise(resolve => {
        ...
        const params = {
            parentId: this.parmasKey?.parentId || value,
            includeChild: this.parmasKey?.includeChild || '-1',
            permissionTypeParam: this.parmasKey?.permissionTypeParam || '-1'
        };
        let { treeData = [] } = this.state;
        if (treeNodeData) {
            params.parentId = treeNodeData[value];
            params.includeChild = treeNodeData?.includeChild;
            params.permissionTypeParam = treeNodeData?.permissionTypeParam;
        }
        this.refresh(params,
            newData => {
                ...
                if (treeNodeData) {
                    ...
                    this.insertChildrenNode(treeNodeData[value], newTreeData, (treeData) => {
                        ...
                        // console.log("插入请求来的节点进入父节点结束...", newTreeData)
                        resolve({
...
const onSelect = function (nodeData) {
    ...
    if (_canExpand) {
        ...
        if (nodeData.node[children] && nodeData.node.children && nodeData.node[children].length) {
