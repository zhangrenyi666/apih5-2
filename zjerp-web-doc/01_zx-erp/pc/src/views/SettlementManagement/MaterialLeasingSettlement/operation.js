...
class operation extends Component {
    ...
    componentDidMount() {
        const { title, workId } = this.props.btnCallbackFn.getSelectedRows()[0];
        const { apiName } = this.props;
        if (workId) {
            this.props.myFetch(apiName, { apiName: 'getZxCtSuppliesLeaseSettlementListFlowDetail', apiType: 'POST', flowId: 'ZxCtSuppliesLeaseSettlement', title: title, workId: workId }).then(({ success, message, data }) => {
                if (success) {
                    this.setState({
        ...
        return (
            <div style={{ padding: '10px' }}>
                <div>
                    ...
                    {
                        flowWebUrl ? <div>
                            <iframe width="100%" height={ContentH * 0.5} frameBorder={0} src={flowWebUrl} title="navigation"></iframe>
                        </div> : null
                    }
                </div>
            </div>
