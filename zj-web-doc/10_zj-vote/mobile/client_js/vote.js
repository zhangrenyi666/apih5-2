...
$(function () {
	...
	window.login(function () {
		...
		$('#voteId').val(window.getUrlParam('voteId'))
		$('#previewFlag').val(window.getUrlParam('previewFlag'));
...
function initData(startNum, pageNumFlag) {
	...
	var countNumberDom = document.getElementById('countNumber');
	if (countNumberDom) {
		countNumberDom.innerHTML = 0;
	}
	//	var reg = new RegExp("(^|&)" + "previewFlag" + "=([^&]*)(&|$)", "i");
	//	var r = window.location.search.substr(1).match(reg);
	//	var previewFlag2 = unescape(r[2]);	//投票ID
	//	alert(previewFlag2);
	var previewFlag = $("#previewFlag").val();
	// var voteId = $("#voteId").val();
	var voteId = window.getUrlParam('voteId');
	var previewFlag = window.getUrlParam('previewFlag');
	...
	window.myAjax('getVoteDetailed', params, function (result) {
		if (previewFlag == 1) {
			voteInfo(result);
			candidate(result)
			questions(result)
			...
			if (!readOnly || (result.data.showFlag == 1 && readOnly)) {
				voteInfo(result);
				candidate(result)
				questions(result)
			}
...
function voteInfo(result) {
	obj = result.data;
	voterList = result.data.voterList;
	$str = '';
	...
	$str += "    </li>";
	//$str+="    <li>";
	//$str+="        <div class='pro_info_tit'>投票规则:</div>";
	//$str+="        <div class='pro_info_con'><a href=''>点击查看</a></div>";
	//$str+="    </li>";
	//    $str+="    <li>";
	//    $str+="        <div class='pro_info_tit'>参与人数:</div>";
	//    $str+="        <div class='pro_info_con'>10/20</div>";
	//    $str+="    </li>";
	$str += "</ul>";
...
function candidate(result) {
	apiUrl = window.api;
	obj = result.data.voteCandidateList
	var n = result.data.startNum;
	if ((n + 1) == obj.length) {
		$("#conItem").hide();
		$("#conItem2").show();
	}
	...
	$str += "</ul>";
	$str += "</div>";
	//$str+="<div class='per_info_m hide'>";
	//$str+="<div class='per_info_mt'>个人简介:</div>";
	//$str+="<p class='per_info_mp'>" + obj[n]["profiles"] + "</p>";
	//$str+="</div>";
	//$str+="<div class='per_info_b' data-state='0'>展开更多</div>";
...
function setCountNUmber() {
	...
	for (var i = 0; i < queItemList.length; i++) {
		...
		if ($(queItem).find("input[type=radio]").length > 0) {
			...
			if (radio.length > 0) {
				var _content = radio.attr('content') ? Number(radio.attr('content')) : 0;
				count += _content;
			}
		}
	}
	if (countNumberDom) {
		countNumberDom.innerHTML = count
	}
}
...
function questions(result) {
	$str = '';
	obj = result.data.voteQuestionsList
		...
		if (value.questionsDifCode == "1") {
			...
			$.each(objDetails, function (m, details) {
				...
				if (details.answerFlag == "1") {
					$str += "            <label>" + questionsNo(value.questionsNo, m) + "<input type='radio' onchange='setCountNUmber()' name='" + n + "'  checked='checked' content = '" + details.content + "' value='" + details.questionsId + "_" + details.questionsDetailsId + "_" + details.content + "'><span class='que_opt'>" + details.content + "分</span></label>";
				}
				else {
					// onchange='radioChange'
					$str += "            <label>" + questionsNo(value.questionsNo, m) + "<input type='radio' onchange='setCountNUmber()' content = '" + details.content + "' name='" + n + "' value='" + details.questionsId + "_" + details.questionsDetailsId + "_" + details.content + "'><span class='que_opt'>" + details.content + "分</span></label>";
				}
				if (m + 1 % 3 == 0) {
					$str += "        </dd>";
				}
			...
			$.each(objDetails, function (m, details) {
				$str += "       <dd>";
				if (details.answerFlag == "1") {
					$str += "            <label style='flex-basis:100%'>" + questionsNo(value.questionsNo, m) + "<input type='radio' name='" + n + "' checked='checked' value='" + details.questionsId + "_" + details.questionsDetailsId + "_" + details.content + "'><span class='que_opt'>" + details.content + "</span></label>";
				}
				else {
					$str += "            <label style='flex-basis:100%'>" + questionsNo(value.questionsNo, m) + "<input type='radio' name='" + n + "' value='" + details.questionsId + "_" + details.questionsDetailsId + "_" + details.content + "'><span class='que_opt'>" + details.content + "</span></label>";
				}
				$str += "        </dd>";
			});
			$str += "   </dl>";
...
function updateData(pageNumFlag) {
	...
	if (unfilled.length > 0 && previewFlag != 1) {
		...
		if (previewFlag == 1) {
			...
			window.myAjax('addAnswerInfo', params, function (result) {
				if (pageNumFlag == 2) {
					alert('投票成功！');
					history.go(-1)
					// WeixinJSBridge.call('closeWindow');
					...
					$("#startNum").val(result.data.startNum);
					initData(result.data.startNum, pageNumFlag);
				}
