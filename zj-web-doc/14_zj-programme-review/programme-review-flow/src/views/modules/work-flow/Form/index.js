...
class Home extends Component {
    ...
    openFlowByPre() {
        ...
        const { myFetch,form,rowData } = this.props;
        ...
        myFetch("openFlow",newBody).then(({ success,data,message }) => {
            ...
            if (success) {
                ...
                if (nodeVars && nodeVars.fileOperationFlag === "2") {
                    attachmentListName.push({
                        // type: "cameraByNoName",
                        type: "files",
                        disabled: true,
                ...
                if (nodeVars && nodeVars.opinionField && flowButtons && flowButtons.length) {
                    ...
                    if (nodeVars.opinionField === "opinionField7") {
                        flowFormColumn.push({
                            type: "string",
                            hide: true,
                            label: "公司总工常用意见标识",
                ...
                myFetch("createOpenFlow",newBody).then(
                    ({ success,message,data }) => {
                        ...
                        if (success) {
                            ...
                            if (createOpenFlowCallback) {
                                createOpenFlowCallback({ props: this.props,res: data })
                            }
                        } else {
                            Toast.fail(message,2);
                        }
