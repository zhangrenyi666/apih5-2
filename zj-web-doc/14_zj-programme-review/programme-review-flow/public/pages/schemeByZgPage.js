window.schemeByZgPage = {//珠海大屏项目桥梁进度
    ...
    actionBtns: [
        {
			...
			onClick:(obj)=>{
                ...
                if(obj.selectedRows[0].chiefEngineerExamineType === "0" || obj.selectedRows[0].chiefEngineerExamineType === "1" ){
            ...
            formBtns: function (args) {
                ...
                if (rowData.chiefEngineerExamineType != "2") {
                    return [
                        {
                            name: 'cancel', //关闭右边抽屉
                ...
                return [
                    ...
                    },
                    {
                        ...
                        onClick: function (obj) { //此时里面会多一个response
							const { fetch,msg } = obj.btnCallbackFn;
							obj._formData.chiefEngineerExamineResult = "1";
                    ...
                    },
                    {
                        ...
                        onClick: function (obj) { //此时里面会多一个response
							const { fetch,msg } = obj.btnCallbackFn;
							obj._formData.chiefEngineerExamineResult = "0";							
                            fetch('updateZjSchemeConfirmation',{ ...obj._formData },function ({ data,success,message }) {
                                if (success) {
                                    var recordid = {
                                        recordid: data.recordid
    ...
    formConfig: [
        ...
        },
        {
            table: {
                title: '处理状态', //表头标题
                dataIndex: 'chiefEngineerExamineType', //表格里面的字段
                key: 'chiefEngineerExamineType',//表格的唯一key  
                render: (data) => {
                    let r = "未提交";
                    switch (data) {
                        case "0":
            ...
            form: {
                type: 'select',
                label: '处理状态',
                field: 'chiefEngineerExamineType', //唯一的字段名 ***必传
                placeholder: '请选择',
                multiple: false, //是否开启多选功能 开启后自动开启搜索功能
                showSearch: false, //是否开启搜索功能 (移动端不建议开启)
                optionData: [//默认选项数据//可为function (props)=>array
