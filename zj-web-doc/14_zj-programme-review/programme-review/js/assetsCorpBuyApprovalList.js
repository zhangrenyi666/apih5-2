        ...
        },
        {
            ...
            "render": function (data) {
                ...
                if (data && data.reviewState !== "") {
                    a = '<a style="color:blue;"  onclick="myOpenByState(\'方案审批详情\',\'' + data.launchId + '\',\'' + data.schemeLevel + '\',\'' + data.workId + '\',\'' + data.schemeLevel + '\',\'' + data.reviewState + '\')" href="javascript:void(0);">' + data.schemeName + '</a>'
                } else {
                    a = data.schemeName
                }
                return a;
...
$("body").on("click", "button", function () {
    ...
    switch (name) {	
        ...
        case "launch":
            if (checkedData.length == 1) {
                if (checkedData[0].reviewState === "") {
					//myFlowOpen('方案发起', checkedData[0].launchId, 'programmeLaunch',checkedData[0].schemeLevel)
        ...
        case "addCrop":
            if (checkedData.length == 1) {
                if (checkedData[0].reviewState === "") {
		    myFlowOpen('方案发起', checkedData[0].launchId, 'approve',checkedData[0].schemeLevel)					
                } else {
                    layer.alert("已发起审批的不可再发起！", { icon: 0, }, function (index) {
                        layer.close(index);
                    });
...
var myOpenLayer;
function myFlowOpen(title, id, url,schemeLevel) {
	var flowId;
       if(schemeLevel == "2"){
    ...
    var options = {
        type: 2,
        title: title,
        content: url + ".html?id=" + id  +"&code=" +code +"&flowId=" +flowId
    }
...
function myOpen(title, id, url, id2) {
    var options = {
        type: 2,
        title: title,
        content: url + ".html?id=" + id + "&id2=" + id2+"&code=" +code
    }
    myOpenLayer = layer.open(options)
    layer.full(myOpenLayer)
}

function myOpenByState(title, id, state, workId ,schemeLevel ,reviewState) {
	var flowId;
	var url;
	if(state == '3'){
    ...
    var options = {
        type: 2,
        title: title,
        content: url + ".html?id=" + id + "&code=" + code + "&flowId=" + flowId + "&workId=" + workId + "&reviewState=" + reviewState + "&btnFlag=1"
    }
    myOpenLayer = layer.open(options)
    layer.full(myOpenLayer)
}