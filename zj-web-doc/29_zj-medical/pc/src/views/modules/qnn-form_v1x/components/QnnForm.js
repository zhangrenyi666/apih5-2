...
const qnnFormCom = function (props) {
    ...
    let _formConfig = [...formConfig].map(item => {
        ...
        if (canAddForm) {
            if (
                ...
                // console.log(initialValue[props.formBlockIndex][_item.field])
                _item.initialValue = initialValue[props.formBlockIndex][_item.field] || _item.initialValue;
            }
        } else {
    ...
    return (
        <div>
            <QnnForm wrappedComponentRef={(me) => {
                //开发中...
                console.assert(props.setFormBlockObj,'setFormBlockObj方法不存在---请检查')
                if (me && props.setFormBlockObj) { 
                    props.setFormBlockObj(props.field,me,props.formBlockIndex)
                }
            }} {...config} />
        </div>
    );
