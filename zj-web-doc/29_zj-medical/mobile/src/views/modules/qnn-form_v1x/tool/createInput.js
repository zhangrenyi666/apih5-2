...
const createInput = function (_params,index) {
    ...
    const { headers = {},componentsKey } = this.props; //,form
    const { token = "" } = headers;
    //参数的统一处理
    ...
    const _props = {
        ...
        //开发中...
        //用于给给组件设置一个fieldsObj对象
        //每个对象都应该存在setValue、getValue、clearValue这三个方法
        setFieldsObj: (field,me) => {
            if (!this.fieldsObj) { this.fieldsObj = {} };
            console.assert(field,'setFieldsObj方法必须传入field参数！！！')
            this.fieldsObj[field] = me;
        }
    };
            ...
            return (
                <div>
                    <QnnFormBlock
                        ...
                        //设置值到父级的方法name规则`${field}InitialValue`
                        ...
                        //用于给qnn-form设置一个formBlocks对象
                        //formBlocks对象中存的的所有表单块
                        setFormBlockObj={(field,me,index) => {
                            if (!this.formBlocks) { this.formBlocks = {} };
                            if (JSON.stringify(index)) {
                                //可增减的表单块 
                                if (!this.formBlocks[field]) { this.formBlocks[field] = {} };
                                this.formBlocks[field][index] = me;
                            } else {
                                //普通表单块
                        ...
                        //用于获取表单块对象、
                        getFormBlockObj={(field)=>{
                            return this.formBlocks[field]
                        }}

                        //删除可增删的表单块中的表单
                        delFormBlockObjByCanAdd={(field,delIndex) => {
                            // delete this.formBlocks[field][delIndex];
                            this.formBlocks[field][delIndex] = null;
                            console.log('要删除的字段和索引:', field,delIndex)
                            console.log('删除后的表单块:', {...this.formBlocks})
                        }}
                    />
