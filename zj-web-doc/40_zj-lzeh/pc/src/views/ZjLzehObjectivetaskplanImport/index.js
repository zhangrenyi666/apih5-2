...
import { Input, Space, Modal, Button, Upload, message, } from 'antd';
...
const config = {
    antd: {
        rowKey: function (row) {
            return row.zjLzehProduceTaskPlanItemId
        },
        size: 'small'
    },
    drawerConfig: {
...
class index extends Component {
    ...
    confirm = () => {
        ...
        if (fileList) {
            const parms = {
                fileList: fileList,
                zjLzehProduceTaskPlanId: this.props.parent.primaryKey
            }
            this.props.parent.props.myFetch('importProduceTaskPlanItem', parms).then(
                (success) => {
                    if (success) {
    ...
    exportFilesFunc = async () => {
        ...
        let params
        const fileName = `生产目标任务计划${moment().format('YYYYMMDD')}`
        ...
        confirm({
            ...
            centered: true,
            onOk: () => {
                download(`${domain}exportProduceTaskPlanItem`, params, { token })
            }
        });
    }
        ...
        return (
            <div>
                ...
                <QnnTable
                    ...
                    fetchConfig={
                        {
                            apiName: 'getZjLzehProduceTaskPlanItemList',
