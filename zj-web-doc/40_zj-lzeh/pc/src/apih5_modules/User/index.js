...
async function userInfoMe(Authorization) {
    ...
    Toast.error(`请求用户信息失败，失败信息：${message}。请刷新页面重新尝试登录！`, 5);
    return;
}
...
const login = (action) => { //登录
    return new Promise((resolve, reject) => {
        ...
        const loginApiGetStr = !isNewVer ? "login" : `${apis["login"]}?grant_type=${grant_type}&client_id=${client_id}&client_secret=${client_secret}&username=${action.userId}&password=${action.userPwd}`;
        ...
        MyFetch(loginApiGetStr, body, "1", !isNewVer).then(async (result) => {
            ...
            if (success) {
                if (!data["userInfo"]) {
                    ...
                    if (!userInfo) return; //没返回信息直接停止下面逻辑
