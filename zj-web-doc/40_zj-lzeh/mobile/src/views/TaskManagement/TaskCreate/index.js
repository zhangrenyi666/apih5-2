...
class index extends Component {
    ...
    getData() {
        this.props.myFetch('getZjLzehTempTaskManageTreeList', {
            // this.props.myPublic.androidApi?.userInfo?.userId
            // JSON.parse(this.props.myPublic.androidApi?.getStrUserLoginInfo?.())?.userInfo?.
            sort: 0,
            allotPersonId: this.getAndroidApiUserInfo('userId')
    ...
    addForChildren() {
        ...
        if (rowData.length === 1 && rowData[0].status === '0' && !rowData[0].parentId) {
            return <Button
                size={'small'}
                type="primary" inline
                onClick={() => {
    ...
    returnStatusDisabled() {
        ...
        if (rowData.length === 1 && rowData[0].status === '3' && rowData[0].createUserName === this.getAndroidApiUserInfo('realName') && NodeTree.isCommitOk) {
            return <Button
                size={'small'}
                type="primary" inline
                onClick={() => {
    ...
    submittedDisabled() {
        ...
        if (rowData.length === 1 && !rowData[0].parentId) {
            if (rowData[0].children.length) {
                if (rowData.length === 1 && (rowData[0].status === '3' || rowData[0].status === '0') && rowData[0].allotPerson === this.getAndroidApiUserInfo('realName') && NodeTree.isCommitOk) {
                    return <Button
                        size={'small'}
                        type="primary" inline
                        onClick={() => {
            ...
            <div className={s.root}>
                <div>
                    <NavBar
                        ...
                        icon={<Icon type="left" />}
                        onLeftClick={() => {
                            // console.log(this.props.myPublic?.closeActivity?.())
                        ...
                        <QnnForm
                            ...
                            formConfig={[
                                ...
                                },
                                {
                                    ...
                                    type: 'date',
                                    disabled: true,
                                    hide: detailViewData.status === '3' || detailViewData.status === '4' ? false : true
                                },
                                {
                                    label: '分配对象',
                                    field: "launcherList",
                                ...
                                },
                                {
                                    ...
                                    // (detailViewData.status === '4' || detailViewData.status === '3') 
                                ...
                                },
                                {
                                    ...
                                    disabled: true,
                                    // disabled: modalType === 'detail' ? true : false,
                                    // hide: detailViewData.status === '0' || detailViewData.status === '2' ? true : false
                                    hide: detailViewData.status === '3' || detailViewData.status === '4' ? false : true
                                },
                                {
                                    label: '状态',
                        ...
                        <Flex direction={'row'} style={{ width: '100%', position: 'absolute', bottom: 30, height: 30 }} justify={'around'}>
                            ...
                            {modalType === 'detail' ? null : <Button size={'small'} type="primary"
                                ...
                                onClick={() => {
                                    ...
                                    if (modalType === 'addChildren') tableChildren.parentId = detailViewData.zjLzehTempTaskManageId
