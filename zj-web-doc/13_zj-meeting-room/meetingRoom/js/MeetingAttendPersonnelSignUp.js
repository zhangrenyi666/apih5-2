    ...
    "columnDefs": [//表格列配置
        ...
        },
		{
			...
			"render": function (data) {
                ...
                html += '<a href="javascript:void(0);" class="dropDown_A" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true" onclick="myOpen(\'查看人员\',\'' + data.recordid + '\',\'' + 'zjMeetingAttendPersonnelSignUpDetail' + '\')">查看人员</a>';
                html += '</span>'
                return html;
            }
        }
...
$("body").on("click", "button", function () {
    ...
    switch (name) {
        case "export"://导出
            if (checkedData.length == 0) {
                ...
                layer.alert("确定导出此数据？", { icon: 0, }, function (index) {
                    l.ajax('zjMeetingSingUpExportExcel',checkedData[0],function(res){
		...
		/*
            ...
            layer.alert("确定导出此数据？", { icon: 0, }, function (index) {
                l.ajax('zjMeetingSingUpExportExcel', params, function (res) {
                    layer.alert("导出成功！", { icon: 0 }, function (index) {
                        layer.close(index);
                        window.location.href = res;
                    })
