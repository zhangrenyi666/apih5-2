...
$("body").on("click", "button", function () {
    ...
    switch (name) {
        ...
        case "signUp":
            if (checkedData.length == 1) {
				window.location.href = window.apih5.domain+"downloadMeetingRoomSingInQrcode?qrcodeName="+checkedData[0].signUpQrcode;			
            } else if (checkedData.length == 0) {
                layer.alert("未选择任何项", { icon: 0, }, function (index) {
                    layer.close(index);
                });
        ...
        case "signIn":
            if (checkedData.length == 1) {
				window.location.href = window.apih5.domain+"downloadMeetingRoomSingInQrcode?qrcodeName="+checkedData[0].signInQrcode;	
            } else if (checkedData.length == 0) {
                layer.alert("未选择任何项", { icon: 0, }, function (index) {
                    layer.close(index);
                });
