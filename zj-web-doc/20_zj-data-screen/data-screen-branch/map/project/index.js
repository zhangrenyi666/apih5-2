$(function () {
    ...
    var global = {
        orgName: decodeURI(window.w.getUrlParam('orgName')) || '', //项目名
        cityName: decodeURI(window.w.getUrlParam('cityName')) || '' //城市名
    }
    ...
    var bMap = {
        addMarker: function (point) {
            ...
            //     window.getData.getMarkerProjectList('apiname', {}, function (data) {
            //         _this.markerClick(data, e.point);
            //     });
            // });
        },
        // markerClick: function (data, point) {
        //     //标注被点击 
        ...
        drawPBorder: function () {
            ...
            //市轮廓渲染
            bdary.get(global.cityName,function (rs) {       //获取行政区域   
                var count = rs.boundaries.length; //行政区域的点有多少个 
                for (var i = 0; i < count; i++) {
                    var ply = new BMap.Polygon(rs.boundaries[i],
                        {
        ...
        init: function () {
            var _this = this;
            //从网址中取出省份经纬度
            // http://localhost:8000/map/project/index.html?cityName=北京 
            //将地址的省名转为坐标点point 
            var myGeo = new BMap.Geocoder();
            // 将地址解析结果显示在地图上,并调整地图视野
            myGeo.getPoint(global.cityName,function (point) {
    ...
    var render = {
        ...
        autoSize: function () {
            ...
            //字体大小自适应
            //排名
            $('.ranking_con .title').css({
                "font-size": dw / 100 * 1.2 + 'px'
            })
    ...
    var refresh = function () {
        ...
        //第一块的数据
        getData.getProjectOneBlankData('getBranchProjectDetailedInformationList',{ orgName: global.orgName },function (data) {
            indexFn.renderProjectOneBlankData(data);
        })
        //第二块的数据
        getData.getDataIndexData(render.ins['dataIndex'],'getBranchProjectOperatingIndicatorsList',{ orgName: global.orgName });
        // for (var i = 0; i < quanIdArr.length; i++) {
        //     var _id = quanIdArr[i];
        //     getData.getQuanData(render.ins[_id], _id, '公路', '#35ffff', 'getProvinceZjDpEngineeringType', { provinceName: global.province });
        // }

        //地图标注
        // getData.getMarkData('apiName', { province: global.province }, function (data) {
        ...
        //     getPoint(data[index]);
        // })
        getData.getMonthTrendDiagramData('getBranchMonthlyTrendList',{ orgName: global.orgName },function (data) {//月趋势图
            indexFn.renderMonthTrendDiagram(data);
        });
        ...
        },'getBranchProjectResourceAllocation',{ orgName: global.orgName });
        getData.getWisdomData(function (data) {
            indexFn.renderNavCon(data);
        });
        // //第一块的数据
        // getData.getOneBlankData('getProvinceProjectInformationList', { provinceName: global.province }, function (data) {
