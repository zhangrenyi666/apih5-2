...
window.w = {
    ...
    getCookie: function (cname) {
        ...
        return w.cookie[cname];
    },
    ajax: function (apiName,params,success) {
        var params = params || {};
        $.support.cors = true;

        var topOrgId = window.w.getParentUrlParam('topOrgId');
        if (topOrgId) {
            params.topOrgId = topOrgId;
        }
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result.code === "3007") {
                    ...
                    w.ajax("user/refreshAccessToken",{ token: w.getCookie('code') },function (getTokendata,getTokenmessage,getTokensuccess) {
                        ...
                        if (result.success) {
                            //重新设置token
                            w.setCookie('code',result.data,30);

                            w.ajax(apiName,params,function (_data,_message,_success) {
    ...
    getParentUrlParam: function (k,windowObj) {//获取地址栏参数，k未键名
        var windowObj = windowObj || window
        var m = new RegExp("(^|&)" + k + "=([^&]*)(&|$)");
        var r = windowObj.parent.location.search.substr(1).match(m);
        if (r != null) return decodeURI(r[2]); return null;
    },
    mapData: [
