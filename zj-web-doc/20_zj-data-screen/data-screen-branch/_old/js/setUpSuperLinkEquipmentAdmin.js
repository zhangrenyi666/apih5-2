...
var pager = $("#pager").page({
    remote: {// ajax请求配置
        url: l.getApiUrl("getZjSuperLinkEquipmentOaMemberList"),
        params: {
        },
...
var detailLayer = $('#detailLayer').detailLayer({
    ...
    conditions: [
		...
		{
            type: "pickTree",
            name: "superLinkEquipmentOaMember",
            label: "权限人员",
			must: true,
            pickType: {
    ...
    onSave: function (obj, _params) {
        l.ajax('updateZjSuperLinkEquipmentOaMember', _params, function (data) {
            pager.page('remote')
            obj.close()
        })
    },
    onAdd: function (obj, _params) {
        l.ajax('addZjSuperLinkEquipmentOaMember', _params, function (data) {
                pager.page('remote')
                obj.close()
            })
    }
    ...
    switch (name) {
        ...
        case "del":
            if (checkedData.length == 0) {
				...
				l.ajax("batchDeleteUpdateZjSuperLinkEquipmentOaMember", checkedData, function () {
					pager.page('remote')
				})
				layer.close(index);
            });
