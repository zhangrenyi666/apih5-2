...
var getData = {
    ...
    getMapData: function (mapI) {
        ...
        w.ajax('getDpCountryMapList', {}, function (data, message, success) {
            if (success) {
    ...
    getRankingData: function (rankingI, name) {
        ...
        //     }
        // }
        w.ajax('getDpCompanyRankingsList', {}, function (data, message, success) {
    ...
    getDataIndexData: function (manageI, apiName, params) {
        apiName = apiName || 'getDpOperatingIndicatorsList';
        if (!loading.manageI) {
    ...
    getQuanData: function (quanI, id, name, preColor, apiName, params) { //图标实例* id* 中文名 进度条颜色
        // console.log(apiName)
        var apiName = apiName || 'getDpEngineeringTypeList';
        if (!loading[id]) {
    ...
    getBannerData: function (name, cb, apiName, params) { //传入标识返回哪个bar的值
        ...
        var apiName = apiName || 'getDpDynamicInformationList';
        w.ajax(apiName, params, function (data, message, success) {
            if (success) {
                if (data) {
                    cb(data[name]);
    ...
    getConfigData: function (cb, apiName, params) { //传入标识返回哪个bar的值
        ...
        var apiName = apiName || 'getDpResourceAllocationList';
        w.ajax(apiName, params, function (data, message, success) {
            if (success) {
                cb(data);
            }
