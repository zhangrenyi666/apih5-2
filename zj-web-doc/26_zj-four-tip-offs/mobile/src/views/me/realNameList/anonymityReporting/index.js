...
class AnonymityReporting extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: false,
    ...
    refresh = () => {
        ...
        myFetch('getZjReportList', { page: page, limit: limit, reportType: '0' }).then(({ data, success, message }) => {
            if (success) {
                this.setState({
                    loading: false,
                    data: data,
                            ...
                            data && data != '' ? data.map((item, index) => {
                                return (
                                    <div key={index} className={s.center} onClick={() => {
                                        dispatch(push(`${mainModule}RealNameListDetail/${item.reportId}/2`));
                                    }}>
...
export default AnonymityReporting;
