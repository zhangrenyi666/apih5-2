                        ...
                        Item={props => {
                            ...
                            return (
                                <div
                                    ...
                                    onClick={() => {
                                        if(item.type === '1'){
                                            dispatch(push(`${mainModule}realNameListJDetail/${item.reportId}`));
                                        }else if(item.type === '2'){
                                            dispatch(push(`${mainModule}realNameListWDetail/${item.reportId}`));
                                        }else if(item.type === '3'){
                                            dispatch(push(`${mainModule}realNameListADetail/${item.reportId}`));
                                        }else if(item.type === '4'){
                                            dispatch(push(`${mainModule}serveDetail/${item.reportId}`));
                                        }
                                    }}
                                >
                                    <div className={s.top}>
                                        <div className={s.topl} style={{color:'#108ee9'}}>{item.typeName}</div>
                                        {/* <div className={s.topr} style={{color:'#108ee9'}}>{item.typeName}</div> */}
                                    </div>
                                    <Divider style={{ margin: "8px 0px" }} />
                                    <div className={s.top}>
                                        <div className={s.topl}>{item.problemDescribe}</div>
                                    </div>
                                    <Divider style={{ margin: "8px 0px" }} />
                                    <div className={s.top}>
                                        <div className={s.topl}>{item.bonusAmount}元</div>
                                        <div className={s.topr}>{moment(item.reportTime).format('YYYY-MM-DD HH:mm:ss')}</div>
                                    </div>
                                </div>
                            );
                        }}
