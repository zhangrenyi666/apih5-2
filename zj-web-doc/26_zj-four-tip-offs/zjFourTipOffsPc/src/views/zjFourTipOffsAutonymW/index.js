...
const config = {
	fetchConfig: {//表格的ajax配置
		apiName: 'pcGetZjViolationReportInfoList',
						...
						formBtns: [
							...
							},
							{
								...
								onClick: function (obj) {
									var index = obj.btnCallbackFn.getActiveKey();
									if (index === '0') {
										obj.btnCallbackFn.fetch('updateZjViolationReportInfo', obj._formData, function ({ data, success, message }) {
											if (success) {
		...
		return (
		 ...
		 upload={this.props.myUpload}
					...
					actionBtns = {[
						...
						// 				cancelText: "取消",
						// 				onOk: () => {
						// 					fetch('batchExposureZjWgwj', body, function ({ data, success, message }) {
						// 						if (success) {
						// 							msg.success(message)
						// 							obj.btnCallbackFn.refresh();
						// 						} else {
							...
							onClick: ({ props, selectedRows, btnCallbackFn }) => {
								...
								if (selectedRows.length) {
									confirm({
										...
										cancelText: "取消",
										onOk: () => {
											var URL = `${domain + "exportZjViolationReport"}`;
											downLoad(URL, selectedRows, { token }, '中交四公局违规违纪有奖举报.zip');
										},
									});
								}else{
									msg.error("请选择数据");
						...
						// },
						{
							...
							label: '删除',
							fetchConfig: {//ajax配置
								apiName: 'batchDeleteUpdateZjViolationReportInfo',
							},
						}
					]}
					{...config}
				...
				<div>
                    ...
                    >
                        <Spin spinning={loading}>
                            ...
                            }} src={`${ureport}preview?_u=file:violationReport.xml&url=${domain}&delFlag=0&reportId=${reportId}`}></iframe>
                        </Spin>
                    </Modal>
                </div>
			</div>
