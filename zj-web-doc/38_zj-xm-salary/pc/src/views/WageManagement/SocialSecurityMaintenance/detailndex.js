                    ...
                    {this.state.treeData.length ? <Tree
                        ...
                        nodeClick={(node, selectedKeys) => {
                            ...
                            this.setState({
                                ...
                                const { data, success, message } = await this.props.myFetch('getZjXmSalarySocialSecuritySchemeListStatus', {});
                                this.setState({
                                    value: node,
                                    tabList: data,
                        ...
                        <Tabs activeKey={activeKey} onChange={this.tabsCallback}>
                            {
                                tabList.map((item, index) => {
                                    return <TabPane tab={item.name} key={(index + 1).toString()}>
                                        {activeKey ? <Detail {...this.props} clickNodeId={clickNodeId} clickNodeName={clickNodeName} zhujiandetail={zhujian} tabId={item.zjXmSalarySocialSecuritySchemeId} tabName={item.name} dod={dod} /> : null}
                                    </TabPane>
                                })
                            }
