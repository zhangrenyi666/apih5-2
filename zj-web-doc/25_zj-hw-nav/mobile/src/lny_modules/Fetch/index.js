...
let { dev, domain, apis, androidApi } = MyPublic;
 
const MyFetch = (apiName = "", body = {}, dataType = dev ? "3" : "1") => {
    //返回Promise
            ...
            case "1"://不验证token
                ...
                fetch(myRequest).then(response => {
                    ...
                    if(result.code === '3003' || result.code === '3004'){ 
                        history.push(`${window.configs.appInfo.mainModule}login`)
                        // resolve({})
                        return;
                    }
                    if (!result.success) {
                        if (result.message === "No message available") {
                            ...
                            MyConsole.error(apiName, JSON.stringify(body), result.message)
                        }
                    }
                    resolve({ ...result })
                }).catch(error => {
