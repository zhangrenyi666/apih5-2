...
const onCheck = function (checkedKeys,{ node, checkedNodes }) {
    ...
    this.setState({
        ...
        valueByTree: selectModal === "1" ? [node.props.eventKey] : checkedKeys.checked
    });
    if (this.props.onChange) {
        if (selectModal === '1') {
            this.props.onChange(node.props.dataRef);
        } else {
            let datas = checkedNodes.map(item => item.props.dataRef)
...
const renderTreeNodes = function (data) {
    ...
    return data.map(item => {
        ...
        if (name === null || name === false) {
            return <TreeNode style={{ display: 'none' }} key={item[value]}></TreeNode>
        }
        if (item[children]) {
            //有子集
            return (
