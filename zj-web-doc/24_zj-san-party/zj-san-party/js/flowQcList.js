...
var pager = $("#pager").page({
	remote: {//ajax请求配置
		url: l.getApiUrl("getZjSanDjFlowQcList"),
		params: {
...
$("body").on("click", "button", function () {
	...
	switch (name) {
	case "add":
		myOpen('Qc成果奖励申请流程发起','','flowQcApprove')
		break;	
		case "del":
		if (checkedData.length == 0) {
		 ...
		 layer.confirm("确定删除，删除的数据不可恢复，请谨慎操作？", { icon: 0, }, function (index) {
                            l.ajax("batchDeleteUpdateZjSanDjFlowQc", checkedData, function () {
                             pager.page('remote')
                            })
                          layer.close(index);
                        });
...
function myOpen1(index, type) {
	...
	case 'xq':
		params = {};
		params.qcId = rowData.qcId;
		l.ajax('getZjSanDjFlowQcDetail',params,function(res){
