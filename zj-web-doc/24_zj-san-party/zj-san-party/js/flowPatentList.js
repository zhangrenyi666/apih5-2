...
var pager = $("#pager").page({
	remote: {//ajax请求配置
		url: l.getApiUrl("getZjSanDjFlowPatentList"),
		params: {
...
$("body").on("click", "button", function () {
	...
	switch (name) {
	case "add":
		myOpen('专利奖励申请流程发起','','flowPatentApprove')
		break;
    case "del":
		...
		layer.confirm("确定删除，删除的数据不可恢复，请谨慎操作？", { icon: 0, }, function (index) {
                            l.ajax("batchDeleteUpdateZjSanDjFlowPatent", checkedData, function () {
                             pager.page('remote')
                            })
                          layer.close(index);
                        });
...
function myOpen1(index, type) {
	...
	case 'xq':
		params = {};
		params.patentId = rowData.patentId;
		l.ajax('getZjSanDjFlowPatentDetail',params,function(res){
