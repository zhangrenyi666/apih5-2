...
var pager = $("#pager").page({
	remote: {//ajax请求配置
		url: l.getApiUrl("getZjSanDjFlowCollectiveList"),
		params: {
...
$("body").on("click", "button", function () {
	...
	switch (name) {
		case "add":
			myOpen('集体奖励申请流程发起', '', 'flowCollectiveApprove')
			break;
		case "del":
			if (checkedData.length == 0) {
				...
				layer.confirm("确定删除，删除的数据不可恢复，请谨慎操作？", { icon: 0, }, function (index) {
					l.ajax("batchDeleteUpdateZjSanDjFlowCollective", checkedData, function () {
						pager.page('remote')
					})
					layer.close(index);
				});
...
function myOpen1(index, type) {
	...
	switch (type) {
		case 'xq':
			params = {};
			params.collectiveId = rowData.collectiveId;
			l.ajax('getZjSanDjFlowCollectiveDetail', params, function (res) {
