...
var pager = $("#pager").page({
	remote: {//ajax请求配置
		url: l.getApiUrl("getZjSanDjFlowProprietaryTechnologyList"),
		params: {
	...
	conditions: [
	             {
	            	 ...
	            	 name: "proprietaryTechnologyId",//输入字段名
	             },
	             {
	            	 type: "text",//
	            	 name: "perName",//
...
$("body").on("click", "button", function () {
	...
	switch (name) {
	case "add":
		myOpen('专有科技成果奖励申请流程发起','','flowProprietaryTechnologyApprove')
		break;	
	case "del":
		if (checkedData.length == 0) {
			...
			layer.confirm("确定删除，删除的数据不可恢复，请谨慎操作？", { icon: 0, }, function (index) {
                            l.ajax("batchDeleteUpdateZjSanDjFlowProprietaryTechnology", checkedData, function () {
                             pager.page('remote')
                            })
                          layer.close(index);
                        });
...
function myOpen1(index, type) {
	...
	case 'xq':
		params = {};
		params.proprietaryTechnologyId = rowData.proprietaryTechnologyId;
		l.ajax('getZjSanDjFlowProprietaryTechnologyDetail',params,function(res){
