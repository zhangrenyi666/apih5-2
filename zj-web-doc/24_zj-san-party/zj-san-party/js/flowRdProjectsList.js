...
var pager = $("#pager").page({
	remote: {//ajax请求配置
		url: l.getApiUrl("getZjSanDjFlowRdProjectsList"),
		params: {
...
$("body").on("click", "button", function () {
	...
	switch (name) {
	case "add":
		myOpen('科技研发项目奖励申请流程发起','','flowRdProjectsApprove')
		break;	
		case "del":
		if (checkedData.length == 0) {
		 ...
		 layer.confirm("确定删除，删除的数据不可恢复，请谨慎操作？", { icon: 0, }, function (index) {
                            l.ajax("batchDeleteUpdateZjSanDjFlowRdProjects", checkedData, function () {
                             pager.page('remote')
                            })
                          layer.close(index);
                        });
...
function myOpen1(index, type) {
	...
	case 'xq':
		params = {};
		params.rdProjectsId = rowData.rdProjectsId;
		l.ajax('getZjSanDjFlowRdProjectsDetail',params,function(res){
