...
var pager = $("#pager").page({
	remote: {//ajax请求配置
		url: l.getApiUrl("getZjSanDjFlowPaperList"),
		params: {
...
$("body").on("click", "button", function () {
	...
	switch (name) {
	case "add":
		myOpen('学术著作、论文奖励申请流程发起','','flowPaperApprove')
		break;	
	case "del":
		if (checkedData.length == 0) {
		 ...
		 layer.confirm("确定删除，删除的数据不可恢复，请谨慎操作？", { icon: 0, }, function (index) {
                            l.ajax("batchDeleteUpdateZjSanDjFlowPaper", checkedData, function () {
                             pager.page('remote')
                            })
                          layer.close(index);
                        });
...
function myOpen1(index, type) {
	...
	case 'xq':
		params = {};
		params.paperId = rowData.paperId;
		l.ajax('getZjSanDjFlowPaperDetail',params,function(res){
