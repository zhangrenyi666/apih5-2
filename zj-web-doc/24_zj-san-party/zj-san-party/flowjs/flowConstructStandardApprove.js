var flowId = l.getUrlParam('id') || "sanDjCredential";//流程模版id
...
$tabTitle.html("施工工艺标准奖励申请")

switch (flowId) {
	case "sanDjCredential":
			...
			tabs: [
				{
					...
					type: "1",
					isMain: "1",
					tbName: "zjSanDjFlowConstructStandard",
					tbIdName: "constructStandardId",
					conditions: [
						{
						...
						},
						{
		    	                	...
		    	                	children: {
		    	                		...
		    	                		placeholder: '请选择',
		    	                		ajax: {
		    	                			api: "getZjSanDjGradeStandardList12",
		    	                			valueName: "recordId",
		    	                			textName: "scoreName",
				...
				switch (dataName) {
					case "launch":
						...
						for (var j = 0; j < flowFormJson.tabs.length; j++) {//第二次遍历flowFormJson.tabs，为了让return起作用所以不用$.each而采用for循环
							...
							//add by lny on 717
							body["apiName"] = "addZjSanDjFlowConstructStandard"//新增
							//add by lny on 717
						}
						//流程发起特殊代码---开始
								...
								if (_s) {//发起成功，返回workId
									loadFlow(_d.flowButtons, {
										otherBody: {
											...
											apiName: "updateZjSanDjFlowConstructStandard",
