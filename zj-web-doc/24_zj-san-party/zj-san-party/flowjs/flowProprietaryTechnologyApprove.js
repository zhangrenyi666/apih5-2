var flowId = l.getUrlParam('id') || "sanDjScientific";//流程模版id
...
$tabTitle.html("专有科技成果奖励申请")

switch (flowId) {
	case "sanDjScientific":
			...
			tabs: [
				{
					...
					type: "1",
					isMain: "1",
					tbName: "zjSanDjFlowProprietaryTechnology",
					tbIdName: "proprietaryTechnologyId",
					conditions: [
						{
						...
						},
						{
		    	                	...
		    	                	children: {
										...
										must: true,
		    	                		ajax: {
		    	                			api: "getZjSanDjGradeStandardList5",
		    	                			valueName: "recordId",
		    	                			textName: "scoreName",
				...
				switch (dataName) {
					case "launch":
						...
						for (var j = 0; j < flowFormJson.tabs.length; j++) {//第二次遍历flowFormJson.tabs，为了让return起作用所以不用$.each而采用for循环
							...
							//add by lny on 717
							body["apiName"] = "addZjSanDjFlowProprietaryTechnology"//新增
							//add by lny on 717
						}
						//流程发起特殊代码---开始
								...
								if (_s) {//发起成功，返回workId
									loadFlow(_d.flowButtons, {
										otherBody: {
											...
											apiName: "updateZjSanDjFlowProprietaryTechnology",
