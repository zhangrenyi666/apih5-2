var flowId = l.getUrlParam('id') || "sanDjSuggest";//流程模版id
...
$tabTitle.html("合理化建议及技术革新成果、五小奖励申请")

switch (flowId) {
	case "sanDjSuggest":
		flowFormJson = {//流程模版json（升级版：根据流程id动态获取，当前前台写死）
			name: "合理化建议及技术革新成果、五小奖励申请",
			titleName: "zcmc",
			tabs: [
				{
					name: "基本信息",
					...
					conditions: [
						...
						},
						{
		    	                	...
		    	                	children: {
		    	                		...
		    	                		placeholder: '请选择',
		    	                		ajax: {
		    	                			api: "getZjSanDjGradeStandardList7",
		    	                			valueName: "recordId",
		    	                			textName: "scoreName",
				...
				switch (dataName) {
					case "launch":
						...
						for (var j = 0; j < flowFormJson.tabs.length; j++) {//第二次遍历flowFormJson.tabs，为了让return起作用所以不用$.each而采用for循环
							...
							//add by lny on 717
							body["apiName"] = "addZjSanDjFlowSuggest"//新增
							//add by lny on 717
						}
						//流程发起特殊代码---开始
								...
								if (_s) {//发起成功，返回workId
									loadFlow(_d.flowButtons, {
										otherBody: {
											...
											apiName: "updateZjSanDjFlowSuggest",
