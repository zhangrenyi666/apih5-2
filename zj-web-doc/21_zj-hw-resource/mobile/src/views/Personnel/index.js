            ...
            <div className={s.root}>
                ...
                >
                    <MyList
                        ...
                        fetchConfig={{
                            apiName: 'getZjHwZyResourcePersonnelList', //后台api
                        }}
                        Item={props => {
                            //列表模板 props里有所有数据
                            ...
                            const item = rowData;
                            const index = rowID;
                            if(item.personnelClass === '0'){
                                item.personnelClass = "管理人员库";
                            }else if(item.personnelClass === '1'){
                                item.personnelClass = "专家人员库";
                            }else if(item.personnelClass === '2'){
                                item.personnelClass = "班组人员库";
                            }else{
                                item.personnelClass = "未知";
                            }
                            return (
                                <div
                                    className={s.center}
                                ...
                                >
                                    <div className={s.top}>
                                        <div className={s.topl}>{item.personnelDepName}</div>
                                        <div className={s.topr}>{item.personnelClass}</div>
                                    </div>
