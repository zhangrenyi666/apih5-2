...
var pager = $("#pager").page({
    remote: {//ajax请求配置
        url: l.getApiUrl("getZjHwzcAssetsNeedApproveList"),
        params: {
			 zcztdm: "1"
        },
    ...
    switch (name) {
        case "proTransfer":
            if (checkedData.length == 1) {
                myOpen('项目调拨申请新增', checkedData[0], 'zjHwzcZcdbApproval')
        ...
        case "corpTransfer":
            if (checkedData.length == 1) {
                myOpen('公司调拨申请新增', checkedData[0], 'zjHwzcZcdbApproval')
            } else if (checkedData.length == 0) {
                layer.alert("未选择任何项", { icon: 0, }, function (index) {
                    layer.close(index);
                });
...
function myOpen(title, data, url) {
    ...
    if(title ==='项目调拨申请新增'){
        id = "projectTransfer";
    }else if (title === "公司调拨申请新增"){
        id = "cropTransfers";
    }
    var options = {
        type: 2,
        title: title,
        content: url + ".html?workId=" + data.workId + "&id=" + id + "&code=" + code + "&sszcid=" + data.recordid
    }
