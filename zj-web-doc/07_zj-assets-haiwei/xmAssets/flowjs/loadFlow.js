(function (w) {
    w.loadFlow = function loadFlow(flowButtons, options) {
        ...
        //1、创建总容器
        var $container = $div.clone().attr("style", "display:none;padding:20px 40px;")//操作弹窗的容器
        //2、创建并添加流程操作按钮
        ...
        //5、创建并添加流程选择提交关闭按钮
        var $submitBtn = $('<button data-name="flowSelectConfirm" data-type="submit" class="btn size-S ml-10 btn-primary" type="button"><i class="Hui-iconfont" >&#xe644;</i> 提交</button>')
        var $closeBtn = $('<button data-name="flowSelectConfirm" data-type="close"  class="btn size-S ml-10 btn-primary" type="button"><i class="Hui-iconfont" >&#xe644;</i> 关闭</button>')
        ...
        //7、绑定按钮事件
        $flowSelectContainer.on("click", '[data-name="flowSelect"]', flowSelect)
        ...
        function createFlowActionBtn(buttonIndex, flowButtons) {
            ...
            var $btn = $('<button class="btn size-S ml-10 btn-primary" type="button"></button >')
                ...
                case "com.horizon.wf.action.ActionSubmit":
                    ...
                    l.ajax("actionFlow", body, function (_d, _m, _s, _r) {
                        if (_s) {
                            ...
                            var $radioBox = $('<input data-name="flowSelect" type="radio"/>')
                            var $checkBox = $('<input data-name="flowSelect" type="checkbox"/>')
                            var $label = $('<label></label>')
                            var $tabSystem = $('<div></div>')
                            $.each(_d.nextNodes, function (index, item) {
                                var $line = $div.clone()
                                var selectName = item.selectName
                                var nodeGroup = item.nodeGroup || "nodeCheckBox"
                                var value = item.selectId
                                var nodeId = item.nodeId
                                var isDone = item.isDone
                                var $tabSystemC = $tabSystem.clone().attr("data-selectValue", value).attr("data-selectName", nodeGroup)
                                var $lineC = $label.clone().text(selectName)

                                if (_d.nextNodes.length > 1) {
                                    var $selectBox = nodeGroup !== "nodeCheckBox" ? $radioBox.clone() : $checkBox.clone()
                                    // $selectBox.attr("name", nodeGroup).attr("disabled", isDone == "true").val(value)
                                    $selectBox.attr("name", nodeGroup).attr("disabled", false).val(value)
                                    if(isDone == "true"){
                                        $lineC.css({
                                            color:'#999'
                                        })
                                    ...
                                    if (authors) {
                                        ...
                                        for (var i = 0; i < 3; i++) {
                                            ...
                                            if (!author || (author && author.isFree != "true")) { iMax++; continue }
                                            $tabBar.append($tabBtn.clone().text(author.authorName))
                                            var $tabCon = $('<div class="tabCon"></div>').attr("id", key)
                                            var conditions = [
                                                {
                                                    type: "pickTree",
                                                    ...
                                                    maxLen: author.selectType == "radio" ? 1 : false,
                                                    otherMemberData: author.initAuthorApih5Old || [],
                                                    pickType: {
                                                        member: "oaMemberList",//成员列表对应接口字段名,false表示不开启该类
                                                        department: author.isUser == "true" ? false : "oaDepartment",//成员列表对应接口字段名,false表示不开启该类
                                                    }
                                                }
                                            ]
                                            loadedNodes[value][key] = $tabCon.detailLayer({
                                                conditions: conditions, customBtnGroup: {
                                                    btns: [],
                                            ...
                                            loadedNodes[value][key].setOpenData({ selectAuthorApih5: author.selectAuthorApih5 || {} })
        ...
        function flowSelect(event) {
            ...
            var $tabSystemC = $('[data-selectName="' + name + '"][data-selectValue="' + value + '"]');
            ...
            if (!loadedNodes[value]) {
                ...
                l.ajax("actionFlow", body, function (_d, _m, _s, _r) {
                    if (_s) {
                        ...
                        actionData["nextNodes"][dataIndex]["authors"] = _d.nextNodes[i].authors
                        actionData["operate"] = _d.operate;
                        actionData["operateClazz"] = _d.operateClazz;
                        actionData["operateText"] = _d.operateText;
                        actionData["operateFlag"] = _d.operateFlag;
                        ...
                        if (authors) {
                            ...
                            for (var i = 0; i < 3; i++) {
                                ...
                                if (!author || (author && author.isFree != "true")) { iMax++; continue }
                                $tabBar.append($tabBtn.clone().text(author.authorName))
                                var $tabCon = $('<div class="tabCon"></div>').attr("id", value.replace("~", "_") + key + dataIndex)
                                var conditions = [
                                    {
                                        type: "pickTree",
                                        name: "selectAuthorApih5",
        ...
        function flowSelectSubmit() {
            ...
            if (hasSelect) {
                for (var index = 0; index < actionData.nextNodes.length; index++) {
                    ...
                    for (var key in authors) {
                        if (authors.hasOwnProperty(key)) {
                            ...
                            if (loadedNodes[selectId][key] && !loadedNodes[selectId][key].getDatas().err.length) {
                                selectAuthorApih5 = loadedNodes[selectId][key].getDatas().data["selectAuthorApih5"]
                            }
                            actionData.nextNodes[index].authors[key]["selectAuthorApih5"] = selectAuthorApih5
                        }
                    }
                }
                var body = { actionData: JSON.stringify(actionData) }
