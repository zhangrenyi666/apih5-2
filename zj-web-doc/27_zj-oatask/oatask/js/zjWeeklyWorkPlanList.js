    ...
    "columnDefs": [//表格列配置
        ...
        },
        {
            ...
            "render": function (data,display,rowData) {
                if(rowData.showFlag === '1'){
                    return '<a style="color:blue;" href="javascript:void(0);" onclick="myOpen2(\'周工作完成情况详情\',\'' + rowData.weekPlanId + '\',\'' + rowData.reportDate + '\')">'+ data +'</a>'
                }else{
                    return data;
                }
            }
        ...
        },
        {
            ...
            "render": function (data) {
                ...
                if(data.showFlag === '1'){
                    html += '<li><a style="color:blue;" href="javascript:void(0);" onclick="myOpen1(\'周工作完成情况填写\',\'' + data.weekPlanId + '\',\'' + data.reportDate + '\')">完成情况填写</a></li>';
                }
                html += '</ul></span>'
                return html;
            }
...
var pager = $("#pager").page({
    remote: {//ajax请求配置
        url: l.getApiUrl("getZjOataskWeekPlanList"),
        params: {},
...
var detailLayer = $('#detailLayer').detailLayer({
    ...
    onSave: function (obj, _params) {
        l.ajax('addZjOataskWeekPlan', _params, function (data, message, success) {
            if (success) {
                pager.page('remote');
                obj.close();
            }
        ...
        case "del":
            if (checkedData.length == 0) {
                ...
                layer.confirm("确定删除？", { icon: 0 }, function (index) {
                    l.ajax("batchDeleteUpdateZjOataskWeekPlan", checkedData, function (data, success, message) {
                        if (success) {
                            pager.page("remote");
                        }
                    });
...
function myOpen(title, id) {
    var options = {
        type: 2,
        title: title,
        content: "zjWeeklyWorkTasksList.html?id=" + id+"&code="+code
    }
    layer.full(layer.open(options))
}
function myOpen1(title, id, reportDate) {
    var options = {
        type: 2,
        title: title,
        content: "zjCompletionOfWeeklyWorkPlanList.html?id="+id+"&reportDate="+reportDate+"&code="+code
    }
    layer.full(layer.open(options))
}
function myOpen2(title, id, reportDate) {
    var options = {
        type: 2,
        title: title,
        content: "zjCompletionOfWeeklyWorkPlanDetail.html?id="+id+"&reportDate="+reportDate+"&code="+code
    }
    layer.full(layer.open(options))
}