        ...
        MyFetch("login", body, "1").then((result) => {
            ...
            if (success) {
                ...
                Toast.offline(message, 2, () => {
                    resolve({ loginInfo: null, redirectInfo: null, logoutInfo })
                })
            }
        })
    })
...
const logout = (pathName) => {//退出
    ...
    return new Promise((resolve) => {
        let fragment = uri.fragment().split(uri.path())[1]
        const data = { ...logoutInfo, moduleName: uri.path(), pathName: pathName ? pathName : fragment, fragment }
        setLogoutInfo(data).then(() => {
            const code = uri.query(true)["code"]
            if ((loginType === "3" || loginType === "4" || loginType === "5") && code) {
