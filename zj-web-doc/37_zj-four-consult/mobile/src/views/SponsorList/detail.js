...
class Index extends Component {
    ...
    render() {
        const { data, flowData, flowWebUrl } = this.state;
                ...
                <div style={{ height: document.documentElement.clientHeight - 45 }}>
                    {data ? <Tabs tabs={data.apih5FlowStatus !== '4' ? [{ title: "基础信息" }, { title: "操作历史" }] : [{ title: "基础信息" }]} swipeable={false}>
                        <div style={{ height: document.documentElement.clientHeight - 90 }}>
                            {
                                data.apih5FlowStatus === '4' ? <FlowFormByzjSjTechnicalServiceEdit {...this.props} flowData={data}/> : <FlowFormByzjSjTechnicalServiceEdit {...this.props}  flowData={data} noButton={true} />
                            }
                        </div>
                        <div style={{ height: document.documentElement.clientHeight - 90 }}>
