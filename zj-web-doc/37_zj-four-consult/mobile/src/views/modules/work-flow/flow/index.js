            ...
            alert('删除', '确定删除么？', [
                { text: '取消', onPress: () => console.log('cancel') },
                {
                    text: '确定', onPress: () => {
                        actionFlow.bind(this)({
                            noVerifyField: true,//不验证表单字段
                            operateObj, visible: false, close: true
                        })
                    }
                },
            ])
...
const actionFlow = function (actionData, callback) {
    ...
    this.qnnForm.getValues(!actionData.noVerifyField).then((values) => {
        values.apiBody.apih5FlowStatus = '0';
        const {
                ...
                footer={[
                    {
                        ...
                        onPress: () => {
                            ...
                            if (workId === 'add' && isMobile() && viewLink) {
                                //新增点击取消直接回列表页面 
                    ...
                    },
                    {
                        ...
                        onPress: () => {
                            ...
                            const foo = curNextNodes.filter(item => { return (item.isSelect === "true" || item.isSelect === true) });
                            let nodeId = null;
                            if (foo.length) {
                                nodeId = foo[0].nodeId;
                            }
