...
class Home extends Component {
    ...
    openFlowByPre() {
        ...
        const { myFetch,noForm,workFlowConfig = {},match,loginAndLogoutInfo: { loginInfo: { userInfo } } } = this.props; //form,
    ...
    render() {
        ...
        const officeDom = (<div id="flow-doc" className={styles.flowDocContainer}>
            ...
            >
                ...
                {
                    uploadRes ? <div style={{ paddingTop: (docIsReadOnly || uploadRes.length >= docMaxNumber) ? "0px" : '6px',}}>{
                        uploadRes.map((item,i) => {
                            return <div className={styles.docItem} key={i}><a onClick={() => {
                                //打开文档
                                this.docFns().openDoc(item)
                            }}>{item.name}</a>
                                {/* 新增时候显示删除按钮 */}
                                {
                                    (workId === "add" && !docIsReadOnly) ? <span className={styles.delBtn} onClick={() => {
