...
const init = () => {
    ...
    return new Promise(resolve => {
        if (window.hotRender && window.hotRenderData.getCorpInfo) {
            //如果是热更新的话 直接将缓存的数据返回即可
            //微信授权jssdk已经授权过了 所以不需要处理了
            resolve({ ...window.hotRenderData.getCorpInfo });
            return;
        }
        ...
        MyFetch("getCorpInfo", { ...body }, "1").then(result => {
            let { success, data, message, code } = result;
            // Toast.hide();
            if (success) {
                //存储值用于下次热更新使用
                window.hotRenderData.getCorpInfo = { ...data };
                ...
                if (
                    enabled &&
                    is_weixin &&
                    (loginType === "3" || loginType === "32" || loginType === "4" || loginType === "31")
                ) {
    ...
    return new Promise((resolve, reject) => {
        if (!dev) {
            ...
            wx.error(res => {
                ...
                notification.error({
                    message: '系统遇到问题，请联系运维人员',
                    description: `微信sdk配置失败：${res.errMsg}`,
                    duration: 3
                });
                reject();
            });
