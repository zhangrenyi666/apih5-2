...
import { Input, Tabs, Timeline, Spin, Modal, Button, message as Msg } from "antd";
...
class Home extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            loading: true,
            // 处理流程时候按钮处理
            btnsCURD: props.btnsCURD || function ({ btns }) {
                return btns;
            },
    ...
    openFlow(_formConfig) {
        ...
        this.setState({
            ...
            navTitle,
            flowFormColumn: _column,
            flowButtons: [{ buttonClass: "cancel" }, { buttonClass: "submit" }, { buttonClass: "createOpenFlow" }],
            isNeedClassifyConfig: false,
            startSettingFlag: "1",
            docIsReadOnly: false
        });
                ...
                this.setState(
                    {
                        ...
                        flowButtons: flowButtons && btnsCURD({
                    ...
                    () => {
                        // this.qnnForm.setValues({ apiBody: _newapiBody })
                    }
                );
            } else {
                Toast.fail(message);
    ...
    submit = () => {
        ...
        const { closeDrawer, refresh } = tableFns;
        this.qnnForm.getValues(true).then((values) => {
            let _apiBody = values["apiBody"];
            _apiBody.apih5FlowStatus = '4';
            myFetch(_apiBody && _apiBody.serviceId ?  apiNameByUpdate : apiNameByAdd,{..._apiBody})
            .then(({ success,message,data }) => {
                    if (success) {
                        Msg.success(message);
                        closeDrawer(false, () => {
    ...
    render() {
        ...
        //新增文档的按钮
        const addDocBtn = (<div style={{ display: docIsReadOnly ? "none" : "block", paddingLeft: "4px" }}>
            <Button onClick={() => { this.docFns().createDoc() }} icon={<PlusOutlined />}>新增文档</Button>
        </div>);
        ...
        if (this.isMobile()) {
            return (
                <Spin spinning={loading} style={{ height: "100vh" }}>
                    <Flex
                        className={`${styles.root} ${styles.rootByMobile} rootByMobile`}
                        direction={"column"}
                    >
                        <NavBar
                            style={{ width: "100%" }}
            ...
            return (
                <Spin spinning={loading}>
                    <div
                        className={`${styles.pcroot} pcroot ${
