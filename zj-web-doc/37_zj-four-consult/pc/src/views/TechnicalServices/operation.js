...
class operation extends Component {
    constructor(props) {
        ...
        this.state = {
            ContentH: '',
            title: props.clickCb.rowData && props.clickCb.rowData.title ? props.clickCb.rowData.title : '',
            workId: props.clickCb.rowData && props.clickCb.rowData.workId ? props.clickCb.rowData.workId : '',
    ...
    componentDidMount() {
        const { title, workId, apiName } = this.state;
        if (workId) {
            this.props.myFetch(apiName, { apiName: 'getZjSjConsultTechnicalServiceFlowDetails', apiType: 'POST', flowId: 'zjSjTechnicalService', title: title, workId: workId }).then(({ success, message, data }) => {
                if (success) {
                    this.setState({
