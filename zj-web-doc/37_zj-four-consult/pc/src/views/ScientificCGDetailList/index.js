...
class index extends Component {
    ...
    render() {
        const { key, TabsData, modelData, visible, visibles, mListFlag, serchData } = this.state;
        return (
                                            ...
                                            Item={props => {
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <div style={{ fontSize: '20px', fontWeight: 'bold', overflow: 'hidden', whiteSpace: 'normal', textOverflow: 'ellipsis', cursor: 'pointer', color: '#0369a7' }} onClick={() => {
                                                            this.props.myFetch('getZjSjConsultScientificInformationDetails', { infoId: item.infoId }).then(({ success, message, data }) => {
                                                                if (success) {
                                                                    if (item.technologicalAchievementsName === '专利') {
                                                                        this.setState({
                                                                            modelData: data,
                                                                            visibles: true
                                                                        }, () => {
                        ...
                        title={
                            <div>
                                ...
                                <div>
                                    ...
                                    <span>
                                        <Tag color={'green'}>
                                            {modelData.technologicalAchievementsName}
                                        </Tag>
                                    </span>
                                </div>
                            </div>
                ...
                {visibles ? <div>
                    <Modal
                        ...
                        title={
                            <div>
                                <div style={{ fontSize: '18px', fontWeight: 'bold' }}>
                                    <span>{modelData.patentName}&nbsp;&nbsp;&nbsp;</span>
                                </div>
                                <div>
                                    <span>{modelData.deptName && modelData.deptName.indexOf(',') !== -1 ? modelData.deptName.split(',').join('→') : modelData.deptName}：{modelData.userName}&nbsp;&nbsp;&nbsp;</span><span>阅读：{modelData.readNum}&nbsp;&nbsp;&nbsp;</span><span><Rate count={1} style={{ fontSize: 25 }} character={<HeartOutlined />} onChange={(value) => {
                                        if (value === 1) {
                    ...
                    >
                        <div style={{padding:'0 20px',fontSize:'16px'}}>
                            <div>专利号：{modelData.patentNo}</div>
                            <div>专利名称：{modelData.patentName}</div>
                            <div>专利类型：{modelData.patentTypeName}</div>
                            <div>授权时间：{moment(modelData.authorizationTime).format('YYYY-MM-DD HH:mm:ss')}</div>
                            <div>申请单位：{modelData.applicant}</div>
                            <div>专利权人：{modelData.patentee}</div>
                            <div>
                                <div>附件：</div>
                                {
                                    modelData.attachmentList && modelData.attachmentList.length ? modelData.attachmentList.map((item, index) => {
