            ...
            },
            {
                ...
                "render": function (data, _, rowData) {
                    if (haveChangerLeader === "1") {
                        return "<input readonly style='width:100%;height:50px;line-height: 40px;font-size:18px;padding:5px;box-sizing: border-box;' type='number' min=0 value='' />";
                    } else {
                        return "<input readonly style='width:100%;height:50px;line-height: 40px;font-size:18px;padding:5px;box-sizing: border-box;' type='number' min=0 value=" + data + " />";
                    }
                }
...
var detailLayer = $('#detailLayer').detailLayer({
    ...
    onAdd: function (obj, _params) {
        ...
        l.ajax("zjXmCqjxAssistantChargeLeaderScore", _params, function (_d, _m, _s) {
            if (_s) {
    ...
    switch (name) {
        case "submit":
            if (state === '7') {
                ...
                l.ajax("zjXmCqjxAssistantExecutiveLeaderScore", rowData, function (_d, _m, _s) {
                    if (_s) {
                        parent.pager.page('remote');
                        layer.alert(_m, { icon: 1, }, function (index) {
                            parent.layer.close(parent.myOpenLayerEKH);
                ...
                rowData.approvalFlag = '0';
                rowData.haveChangerLeader = haveChangerLeader;
                l.ajax("zjXmCqjxAssistantChargeLeaderScore", rowData, function (_d, _m, _s) {
                    if (_s) {
                        parent.pager.page('remote');
                        layer.alert(_m, { icon: 1, }, function (index) {
                            parent.layer.close(parent.myOpenLayerEKH);
...
function myblur(rowData, type, value) {
    ...
    if (type === 'chargeLeaderScore') {
        ...
        if (rowData.executiveLeaderScore !== value.val()) {
            rowData.executiveLeaderScore = value.val();
            l.ajax('updateZjXmCqjxDeptLeaderAssistantDetailed', rowData, function (data, message, success) {
                if (success) {
                    // pager.page('remote');
                    // pagers.page('remote');
