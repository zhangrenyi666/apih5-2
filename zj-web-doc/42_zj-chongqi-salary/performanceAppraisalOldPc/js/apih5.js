        ...
        FormItem.prototype.createDom = function () {
            ...
            switch (that.type) {
                ...
                case "upload"://上传组件唯一
                    ...
                    var $upload = $('<div class="uploadFile" data-uploadUrl="' + (that.uploadUrl ? that.uploadUrl : "") + '" data-projectName="' + (that.projectName ? that.projectName : "") + '" data-name="' + ($me.idName + that.name) + '" data-maxLen="' + that.maxLen + '"><ul class="uploadList"></ul><label style="position: relative;" for="uploadBtn_' + ($me.idName + that.name) + '" name="' + that.name + '" class="btn btn-primary size-S">' + (that.btnName ? that.btnName : '添加') + '<input  id="uploadBtn_' + ($me.idName + that.name) + '" data-fileType=\'' + JSON.stringify(that.fileType ? that.fileType : []) + '\' name="filesName" size="2" class="inputFile"  type="file" /></label><span class="ml-10">' + (that.remind ? that.remind : (that.fileType ? ('上传格式要求：' + JSON.stringify(that.fileType)) : '')) + '</span></div>')
                    $con.append($upload);
                    break
                case "option"://option
