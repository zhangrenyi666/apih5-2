...
function myblur(rowData,type, value) {
    ...
    if (type === 'score') {
        if (rowData.score !== parseFloat(value.val()) && value.val()) {
            ...
            l.ajax('batchAddZjXmCqjxDisciplineAssessmentMember', [rowData], function (data, message, success) {
                if (success) {
                    pager.page('remote');
    ...
    switch (name) {
        ...
        case "submit":
            layer.confirm("确定提交吗？", { icon: 3, }, function (index) {
                l.ajax("zjXmCqjxDisciplineAssessmentSubmit", {disciplineId:disciplineId}, function (data, message, success) {
                    if (success) {
                        parent.pager.page('remote');
                        parent.layer.close(parent.myOpenLayer);
                    }
