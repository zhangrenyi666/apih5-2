...
var detailLayer = $('#detailLayer').detailLayer({
    ...
    onAdd: function (obj, _params) {
        l.ajax("zjXmCqjxStaffAssistantLaunch", _params, function (_d, _m, _s) {
            if (_s) {
    ...
    switch (name) {
        ...
        case "submit":
            l.ajax("checkZjXmCqjxAssistantScoreQualified", { executiveId: id }, function (data, message, success) {
                if (success) {
                    l.ajax("zjXmCqjxStaffAssistantLaunch", { executiveId: id, assessmentType: '3'}, function (_d, _m, _s) {
                        if (_s) {
                            layer.alert(_m, { icon: 1, }, function (index) {
                                parent.pager.page('remote');
                                parent.layer.close(parent.myOpenLayerJH);
