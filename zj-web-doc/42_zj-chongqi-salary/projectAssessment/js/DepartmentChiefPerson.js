    ...
    table = $('#table').DataTable({
        ...
        "columnDefs": [
            ...
            },
            {
                ...
                "render": function (data, _, rowData) {
                    if(!data){
                        return "<input readonly style='width:100%;height:50px;line-height: 40px;font-size:18px;padding:5px;box-sizing: border-box;' type='number' min=0 value='' onblur='myblur(\"" + encodeURI(JSON.stringify(rowData)) + "\",\"" + 'assessmentScore' + "\",$(this))' />";
                    }else{
                        return "<input readonly style='width:100%;height:50px;line-height: 40px;font-size:18px;padding:5px;box-sizing: border-box;' type='number' min=0 value=" + data + " onblur='myblur(\"" + encodeURI(JSON.stringify(rowData)) + "\",\"" + 'assessmentScore' + "\",$(this))' />";
                    }
                }
            },
            {
...
$("body").on("click", "button", function () {
    ...
    switch (name) {
        case "add":
            l.ajax("addZjXmCqjxProjectSecretaryrAssistantDetailed", { executiveId:id, workType:'0' }, function () {
                pager.page('remote')
            })
            break;
        case "adds":
            l.ajax("addZjXmCqjxProjectSecretaryrAssistantDetailed", { executiveId:id, workType:'1' }, function () {
                pagers.page('remote')
            })
            break;
        case "del":
            if (checkedData.length == 0) {
                ...
                layer.confirm("确定删除？", { icon: 0, }, function (index) {
                    l.ajax("batchDeleteUpdateZjXmCqjxProjectSecretaryrAssistantDetailed", checkedData, function () {
                        pager.page('remote')
                    })
                    layer.close(index);
                });
        ...
        case "dels":
            if (checkedDatas.length == 0) {
                ...
                layer.confirm("确定删除？", { icon: 0, }, function (index) {
                    l.ajax("batchDeleteUpdateZjXmCqjxProjectSecretaryrAssistantDetailed", checkedDatas, function () {
                        pagers.page('remote')
                    })
                    layer.close(index);
                });
        ...
        case "submit":
            ...
            if (positions) {
                ...
                if (submitType === 'add') {
                    l.ajax("zjXmCqjxProjectDeptLeaderAssistantFillIn", _params, function (_d, _m, _s) {
                        if (_s) {
