...
var pager = $("#pager").page({
    remote: { //ajax请求配置
        url: l.getApiUrl("getXmZlSafetyRiskListList"),
        params: {},
...
$(document).ready(function () {
    ...
    l.ajax('getXmZlSafetyRiskListLevelList', {
        parentId: '0'
    }, function (res) { //请求所有数据 
        var node; //右键时选择的节点 
        var ztreeFn = {
            ...
            zTreeOnRename: function (e, treeId, treeNode) { //改名回调
                ...
                wxx.folderFlag = '';
                wxx.isParent = '';
                l.ajax('updateXmZlSafetyRiskListLevel', params, function (res) {
                    layer.msg('修改成功');
        ...
        var wxx = {
            ...
            setting: { //默认配置项
                ...
                async: {
                    enable: true,
                    url: l.domain + "getXmZlSafetyRiskListLevelList",
                    autoParam: ["levelId"],
                    dataType: "json",
                    type: "post",
                    ...
                    if (type == 'node') { //增加子节点
                        ...
                        if (canAdd) {
                            ...
                            layer.open({ // 自定义弹出窗口，输入名称的编码
                                ...
                                title: "输入节点名称和编码",
                                btn: ['确定', '取消'],
                                content: "<div class='inputOut'><span class='inputSpan'>名称：</span><input type=\"text\" id= \"inputLevelName\" class=\"newNodeInput\" placeholder=\"输入名称\"/></div></br><div class='inputOut'><span class='inputSpan'>编码：</span><input type=\"text\" id= \"inputLevelCode\" class=\"newNodeInput\" placeholder=\"输入编码\"/></div>",
                                yes: function (index) {
                                    if ($("#inputLevelName").val() === "") {
                                        ...
                                        l.ajax('addXmZlSafetyRiskListLevel', params, function (res) {
                                            layer.msg('新增成功')
                                            // 关闭弹窗
                                            layer.close(index)
            ...
            delNode: function (event, type) { //删除 fn 根据pid判断是不是根节点
                ...
                layer.confirm('确定删除吗？', {
                    ...
                    l.ajax('batchDeleteUpdateXmZlSafetyRiskListLevel', params, function (res) {
        ...
        var detailLayer = $('#detailLayer').detailLayer({
            ...
            onSave: function (obj, _params) {
                l.ajax('updateXmZlSafetyRiskList', _params, function (data) {
                    pager.page('remote')
                    obj.close()
                })
            },
            onAdd: function (obj, _params) {
                _params.levelId = node.levelId,
                    _params.levelNameAll = node.parentNameAll
                _params.levelIdAll = node.parentIdAll
                l.ajax('addXmZlSafetyRiskList', _params, function (data) {
                    pager.page('remote')
                    obj.close()
                })
            }
                ...
                case "del":
                    if (checkedData.length == 0) {
                        ...
                        layer.alert("确定删除？", {
                            ...
                            layer.close(index);
                            l.ajax("batchDeleteUpdateXmZlSafetyRiskList", checkedData, function () {
                                pager.page('remote')
                            })
                        });
