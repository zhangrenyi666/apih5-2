...
//pc端环境下直接跳转pc地址
window._apih5ToPc = function (token, params) {
    if ((navigator.userAgent.match(/(MicroMessenger)/i))) {
        var isMobile = navigator.userAgent.match(/(iPhone|iPod|ios|iPad|Android)/i);
        // isMobile = false;  //测试代码 
        if (!isMobile && window.configs.pcPageUrl && window.configs.pcPageUrl !== "") {
            //pc端环境
            //pc需要跳转页面
            if (window.configs.pcPageUrl) {
                ...
                if (window.configs.pcPageUrl.replace(/\//ig, '') !== curUrl.replace(/\//ig, '')) {
                    ...
                    var toUrl = window.configs.pcPageUrl;

                    //需要注意有#号存在的需要拆分
                    if (token) {
                        var splitUrl = window.configs.pcPageUrl.split('#');
                        if (splitUrl.length >= 3) {
                            alert('configs.pcPageUrl配置格式有误，#号在整段url中只可出现一次');
                            return;
                        }
                        //带了具体地址的路由直接拼上具体路由  未带的拼接上mainModule
                        toUrl = splitUrl[0] + '?webCode=' + token + '#' + (splitUrl[1] ? splitUrl[1] : window.configs.appInfo.mainModule);
                    }


                    var wxUrl = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=" + params.corpId + "&redirect_uri=" + encodeURI(toUrl) + "?" + "&response_type=code&scope=snsapi_base&agentid=" + params.agentId + "&state=STATE#wechat_redirect"; // 在默认浏览器打开redirect_uri，并附加code参数；也可以直接指定要打开的url，此时不会附带上code参数。
