...
//先获取模快数据 然后生成到筛选下拉和表单上面
var initFn = function (initEd) {
    l.ajax("getZjZpEvaluationModuleList",{ assessmentId: assessmentId },function (
        data,
        message,
        success
    ) {
...
initFn(function (moduleList) {  
    //格式化出下拉数据
    var selectOption = []; 
    for (var i = 0; i < moduleList.length; i++) {
        selectOption.push(
            {
                value: moduleList[i].moduleId, //输入字段的值
                text: moduleList[i].moduleName, //显示中文名 
                selected: false //默认是否选中
            }
        )
    }
    var selectOptionByFilter = [
        {
            value: "", //输入字段的值
            text: "全部", //显示中文名
        ...
        columnDefs: [
            ...
            },
            {
                ...
                render: function (data) {
                    ...
                    //自定义输出
                    for (var i = 0; i < moduleList.length; i++) {
                        if (moduleList[i].moduleId === data) {
                            r = moduleList[i].moduleName;
                        }
                    }
                    return r;
                }
    ...
    var form = $("#form").filterfrom({
        conditions: [
            ...
            },
            {
                ...
                name: "moduleType",
                label: "工作模块分类",
                selectList: selectOptionByFilter
            }
        ],
    ...
    var pager = $("#pager").page({
        remote: {
            //ajax请求配置
            url: l.getApiUrl("getZjZpEvaluationList"),
            params: {
                evaluationType: "",
                moduleType: "",
    ...
    var detailLayer = $("#detailLayer").detailLayer({
        ...
        onSave: function (obj,_params) {
            ...
            l.ajax("updateZjZpEvaluation",params,function (data) {
                pager.page("remote");
                obj.close();
            });
        },
        onAdd: function (obj,_params) {
            ...
            params["bmId"] = bmId;
            params["assessmentId"] = assessmentId;
            l.ajax("addZjZpEvaluation",params,function (data) {
                pager.page("remote");
                obj.close();
            });
        }
        ...
        switch (name) {
            ...
            case "del":
                if (checkedData.length == 0) {
                    ...
                    l.ajax("batchDeleteZjZpEvaluation",checkedData,function (
                        data,
                        message,
                        success
                    ) {
