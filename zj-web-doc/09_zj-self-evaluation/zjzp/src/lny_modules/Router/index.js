...
const { URI, appInfo: { mainModule }, androidApi } = MyPublic //
...
const createRouteData = (array, pid = "") => {
    ...
    for (let index = 0; index < array.length; index++) {
        let { hide = false, label = "", defaultPath = "", pathName = "", iconType = "", moduleName = "", redirect = "", children = [], comKey = "" } = array[index];
        let id = (pid ? pid + "_" : "") + index;
        let menuLv = pid.split("_").filter(i => i !== "").length
        if (children.length > 0) {
            let { routeData: _routeData, routeTree: _routeTree } = createRouteData(children, id)
            routeData[id] = { hide, id, pid, menuLv, label, defaultPath: defaultPath || pathName || _routeTree[0].defaultPath || _routeTree[0].pathName || "", pathName, moduleName: moduleName || _routeTree[0].moduleName || "", iconType, comKey, redirect: comKey ? "" : (redirect || children[0].pathName || children[0].redirect || ""), children, routeTree: _routeTree }
            routeData = {
                ...routeData,
                ..._routeData
            }
        } else {
            routeData[id] = { hide, id, pid, menuLv, label, defaultPath: defaultPath || pathName, pathName, moduleName, iconType, comKey, redirect, children: [], routeTree: [] }
...
let NotFounndCom = (props) => {
    const { dispatch, actions: { setRedirectInfo } } = props
    return (
        <div>
            {`找不到该页面${props.location.pathname}，跳转至`}<a onClick={() => {
                dispatch(setRedirectInfo({ moduleName: mainModule, pathName: "/", fragment: "" }))
                if (androidApi) {
                    androidApi.closeActivity()
                } else {
                    window.location.href = new URI().host(new URI().host()).path(mainModule).hash(`${mainModule}`).toString()
                }
            }} >{`首页`}</a>
        </div>
    )
...
const MyComponent = (props) => {
    ...
    const { dispatch, corpInfo, actions: { setRedirectInfo, login, logout }, loginAndLogoutInfo, routerInfo: { routeData: _routeData, curKey } } = props
    ...
    if (curRedirect) {
        ...
        if (!mustLogin && curPathName !== "login") {//直接访问不需要授权的页面
            ...
            return <Com {...props} myFetch={myFetch} myPublic={MyPublic} myLogout={myLogout} />
        } else if (loginInfo && !loginInfo.error && !loginInfo.loading) {//有登录信息
            MyConsole.log("有登录信息,token:", loginInfo.token)
            if (redirectInfo) {//登录后尚未跳转至指定页面的
                MyConsole.log("重定向信息尚未清除")
                let { moduleName: _moduleName = curModuleName, pathName: _pathName = curPathName, fragment = "" } = redirectInfo
            ...
            if (loginType === "6") {
                ...
                if (androidApi) {
                    ...
                    return <div style={{ textAlign: "center" }}>{"请在App内打开."}</div>
                }
            } else if ((loginType === "3" ||  loginType === "4" || loginType === "5" || loginType === "31") && code) {//微信重定向地址
                MyConsole.log("登录类型为[3,4,5]-微信重定携带code")
                dispatch(login({ code, loginType }))//使用code登录
                if (loginInfo && loginInfo.error) {
                    return <div style={{ textAlign: "center" }}>{"微信授权失败，尝试点击"}<a onClick={() => { myLogout() }}>{"重新登录"}</a>{"，或请联系管理员。"}</div>
                } else if (loginInfo && loginInfo.loading) {
                    return <div style={{ textAlign: "center" }}>{"微信授权登录中..."}</div>
                } else {
                    return <div style={{ textAlign: "center" }}>{"马上进行微信授权登录。"}</div>
                }
            } if (loginType === "3" || loginType === "4" || loginType === "31") {//微信oauth2授权
                MyConsole.log("登录类型为[3,4]-微信重定未携带code")
                window.location.href = corpInfo.uri
                return <div style={{ textAlign: "center" }}>{"微信授权请求中..."}</div>
            ...
            MyConsole.log("访问授权页或登录页，但登录登出信息均不存在，设置登出信息")
            myLogout()
            return <div />
        }
    }
                ...
                routes.push(
                    <Route
                        ...
                        component={connect(selectCom)(props => {
                            ...
                            return <MyComponent {...props} routerInfo={{ routeData, routeTree, curKey: key }} />
                        })} />
                )
            }
        }
    ...
    routes.push(
        <Route key={"-2"} component={
            connect(selectCom)(props => {
                ...
                const { dispatch, corpInfo, actions: { setRedirectInfo, login }, loginAndLogoutInfo: { loginInfo, redirectInfo, logoutInfo } } = props
                if (props.location.pathname === "/") {
                    return <Redirect to={realModuleName} />
                } else {
                    if (logoutInfo) {//是否有退出信息
                        const { loginType } = logoutInfo
                        const code = uri.query(true)["code"]
                        if (loginType === "6") {
                            const token = androidApi.getLoginToken()
                            dispatch(login({ code: token, loginType }))//使用token登录
                            return <div />
                        } else if ((loginType === "3" || loginType === "4" || loginType === "5") && code) {//微信重定向地址
                            dispatch(login({ code, loginType }))//使用code登录
                            return <div />
...
const _createRoutes = (dynamicRouterInfo) => {
    ...
    const routerInfo = [].concat(mainRouterInfo, _viewRouterInfo, dynamicRouterInfo)
