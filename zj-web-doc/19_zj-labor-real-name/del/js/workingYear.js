...
var pager = $("#pager").page({
    remote: {//ajax请求配置
        url: l.getApiUrl("getWorkingYearList"),
        params: {
        },
...
var detailLayer = $('#detailLayer').detailLayer({
    ...
    onSave: function (obj, _params) {
        l.ajax('updateWorkingYear', _params, function (data) {
            pager.page('remote')
            obj.close()
        })
    },
    onAdd: function (obj, _params) {
        l.ajax('addWorkingYear', _params, function (data) {
            pager.page('remote')
            obj.close()
        })
    }
            ...
            if (checkedData.length == 0) {
                layer.alert("未选择任何项", { icon: 0, }, function (index) {
				   ...
				   layer.confirm("确定删除？", { icon: 0,}, function (index) {            
					   l.ajax("batchDeleteUpdateWorkingYear", checkedData, function () {
					   pager.page('remote')
				    })
				  layer.close(index);
               });
