...
var detailLayer = $('#detailLayer').detailLayer({
    ...
    onSave: function (obj, _params) {
        l.ajax('updatePerAge', _params, function (data) {
            pager.page('remote')
            obj.close()
        })
    },
    onAdd: function (obj, _params) {
        l.ajax('addPerAge', _params, function (data) {
            pager.page('remote')
            obj.close()
        })
    }
            ...
            if (checkedData.length == 0) {
                layer.alert("未选择任何项", { icon: 0, }, function (index) {
				   ...
				   layer.confirm("确定删除？", { icon: 0,}, function (index) {            
					   l.ajax("batchDeleteUpdatePerAge", checkedData, function () {
					   pager.page('remote')
				    })
				  layer.close(index);
               });
