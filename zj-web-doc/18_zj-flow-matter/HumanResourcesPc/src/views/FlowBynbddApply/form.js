            ...
            formConfig: [
                {
                    ...
                    Component: obj => {
                        return (
                            <Divider style={{ fontSize: '22px',color: '#545456',height: '38px',lineHeight: '38px',margin: '16px 0 0 0',textAlign: 'center' }} orientation="middle">内部调动审批表</Divider>
                        );
                    }
                },
                {
            ...
            <Form
                ...
                btnsCURD={({ btns,flowData }) => {
					...
					if (url === `${myPublic}todoByTran`) {
						...
						if (flowData && flowData.flowNode.nodeId === 'Node2' || flowData && flowData.flowNode.nodeId === 'Node4' || flowData && flowData.flowNode.nodeId === 'Node6' || flowData && flowData.flowNode.nodeId === 'Node8') {
							var printUrl = `${ureport}excel?_u=file:reportZjFlowTranInner.xml&url=${domain}&workId=${flowData.workId}`
							btns.push({ buttonClass: "exprot",buttonFun: null,buttonId: "exprot",buttonName: "导出",icon: null,printUrl: printUrl });
						}
					}
                    ...
                    var fields = fields.map((item) => {
                        ...
                        if (list.apih5FlowStatus === '0') {
                            if (item.children) {
                                ...
                                if (field === 'remarks' || field === 'opinionField6' || field === 'opinionField5' || field === 'opinionField4' || field === 'opinionField3' || field === 'opinionField2' || field === 'opinionField1') {
                                    item.disabled = true;
                                } else {
                                    item.disabled = false;
                                }
