			...
			<div>
				<Form
					...
					btnsCURD={({ btns, flowData }) => {
						if (flowData?.nodeVars?.showPrintFlag === '1' && flowData?.flowId === 'jgDepositPayment') {
							let { myPublic: { domain, appInfo: { ureport } } } = this.props;
							var printUrl = `${ureport}excel?_u=file:zjFlowDepositPaymentForjg.xml&url=${domain}&workId=${flowData?.workId}`
					...
					// 	if (changedValues.apiBody) {
					// 		const arr = allValues.apiBody.depositDetailList_Block;
					// 		var sum = 0;
					// 		for (let i = 0; i < arr.length; i++) {
					// 			if (arr[i].depositMoney || arr[i].depositMoney === 0) {
					// 				let _val = arr[i].depositMoney || 0;
					// 				sum += _val;
					// 				if (sum !== allValues.total) {
					// 					let upperSum = this.changeNumMoneyToChinese(sum);
					// 					props.form.setFieldsValue({ "apiBody.totalMoney": sum,"apiBody.upperTotalMoney":upperSum })
					// 				}
					// 			}
