...
$("body").on("click", "button", function () {
    ...
    switch (name) {
		...
		case "process1":
            if (checkedData.length == 1) {
                myOpen("流程处理", checkedData[0].workId, "zjYyOutSealApplyProcess")
            } else if (checkedData.length == 0) {
                layer.alert("未选择任何项", { icon: 0, }, function (index) {
                    layer.close(index);
                });
		...
		case "process2":
            if (checkedData.length == 1) {
                myOpen("流程处理", checkedData[0].workId, "zjInfoNeedsConfirmProcess")
            } else if (checkedData.length == 0) {
                layer.alert("未选择任何项", { icon: 0, }, function (index) {
                    layer.close(index);
                });
		...
		case "process3":
            if (checkedData.length == 1) {
                myOpen("流程处理", checkedData[0].workId, "zjInfoProjectApplyProcess")
            } else if (checkedData.length == 0) {
                layer.alert("未选择任何项", { icon: 0, }, function (index) {
                    layer.close(index);
                });
