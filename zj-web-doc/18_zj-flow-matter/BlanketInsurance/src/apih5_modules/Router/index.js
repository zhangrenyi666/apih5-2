...
const createRouteData = (array, pid = "", _rootid = "") => {
	let routeData = {};
	let routeTree = [];
	for (let index = 0; index < array.length; index++) {
		let { hide = false, label = "", defaultPath = "", pathName = "", iconType = "", moduleName = "", redirect = "", children = [], comKey = "", value, valuePid, showType, ...other } = array[index];
		let id = (pid ? pid + "_" : "") + index;
		const rootid =  _rootid || _rootid === 0 ? _rootid : id
		let menuLv = pid.split("_").filter((i) => i !== "").length;
		if (children.length > 0) {
			let { routeData: _routeData, routeTree: _routeTree } = createRouteData(children, id, rootid);
			routeData[id] = {
				...other,
				_value: value,
				_valuePid: valuePid,
				...
				menuLv,
				label,
				defaultPath: defaultPath || pathName || _routeTree[0].defaultPath || _routeTree[0].pathName || "",
				pathName,
				moduleName: moduleName || _routeTree[0].moduleName || "",
				iconType,
				comKey,
				redirect: comKey ? "" : redirect || children[0].pathName || children[0].redirect || "",
				children,
	...
	if (isEn) {
		return (
			...
			>
				...
				<p>The routing address：{props.location.pathname}</p>
				<p>
					It will jump to the home page in {1} seconds（
					<a
						onClick={() => {
							tzFn(tzTimer);
						}}
...
const MyComponent = (props) => {
	...
	if (loginInfo && loginInfo?.token) {
		//登录成功之后不一定是跳转路由 也可能是直接获取token跳转到pc端的地址
		//之间跳转的情况就无需渲染移动端了
		if (window._apih5ToPc && window._apih5ToPc(loginInfo.token, corpInfo)) {
			return <div />;
		}
	}
	...
	const myLogout = () => {
		dispatch(logout(curPathName, _routeData[curKey]));
	};
		...
		return MyFetch(...arg).then((result) => {
			if (result.code === "3003" || result.code === "3004") {
				...
				if (androidApi) {
					...
					Modal.alert(isEn ? "Identity information expiration prompt" : "身份信息过期提示", isEn ? "Is the login information out of date?" : "登录信息过期是否重新登录？", [
						{ text: isEn ? "cancel" : "取消", onPress: () => {} },
						{
							text: isEn ? "confirm" : "确定",
							onPress: () => {
								myLogout();
							},
						},
	...
	if (curRedirect) {
		...
		if (!mustLogin && curPathName !== "login") {
			...
			return <Com {...props} myUpload={myUpload} myFetch={myFetch} myPublic={MyPublic} myLogout={myLogout} />;
			...
			if (redirectInfo) {
				...
				return <Com {...props} myFetch={myFetch} myUpload={myUpload} myPublic={MyPublic} myLogout={myLogout} />;
			}
		} else if (logoutInfo) {
			//是否有退出信息
			...
			} else if ((loginType === "3" || loginType === "32" || loginType === "4" || loginType === "5" || loginType === "31") && code) {
