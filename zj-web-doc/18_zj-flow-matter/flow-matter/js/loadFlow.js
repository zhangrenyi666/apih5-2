        ...
        function flowSelectOpen(index, newformData) {
            ...
            switch (buttonClass) {
                ...
                case "com.horizon.wf.action.ActionReject":
                    var lIndex = layer.confirm('确定退回么？', {
                        btn: ['确定', '取消'] //按钮
                    }, function () {
                        body.actionData = JSON.parse(body.actionData);
                        body.actionData.nextNodes = [];
                        body.actionData = JSON.stringify(body.actionData);
                        l.ajax("actionFlow", body, function (_d, _m, _s, _r) {
                            if (_s) {
                                endFn(buttonClass);
                                layer.confirm(_d.operateMsg, {
                                    btn: ['确定'], //按钮 
                                }, function () {
                                    layer.closeAll();
                                    // var i = window.parent.layer.getFrameIndex(); 
                                    window.parent.layer.closeAll();
                                    // window.parent.pager.page('remote');
                                });
                            } else {
