...
function myOpen(title, data, url) {
    ...
    if (data.flowId == 'zjYongYin') {//用印
        ...
        url = 'zjDwSealProcess'
    } else if (data.flowId == 'lianhedudao') {//联合督导申请
        url = 'http://weixin.fheb.cn:99/zj-work-check/jointApplyOtherProcess'
    ...
    } else if (data.flowId == 'zjZcPurchaseCorp') {//中交资产购置
    ...
    } else if (data.flowId == 'zlhzApply') {//战略合作框架协议、联合体协议评审(局)
        url = 'zjFlowBygszlhzApplyEdit'
    } else if (data.flowId == 'gszlhzApply') {//战略合作框架协议、联合体协议评审(各单位)
        url = 'zjFlowBygszlhzApplyEdit'
    } else if (data.flowId == 'ZjMeetingPlanFallInSummary') {//年度会议计划
        url = 'http://weixin.fheb.cn:99/zjMeetingRoom/zjMeetingPlanFallInSummaryProcess'
    } else if (data.flowId == 'ZjMeetingPlanSummary') {//年度会议计划汇总
        url = 'http://weixin.fheb.cn:99/zjMeetingRoom/zjMeetingPlanSummaryProcess'
    }else if (data.flowId == 'sanPerson') {//三公司人事审批
        url = 'http://weixin.fheb.cn:99/PersonApprovalOldPc/returnWorkOutProcess'
    }else if (data.flowId == 'sanMinister') {//三公司人事审批
        url = 'http://weixin.fheb.cn:99/PersonApprovalOldPc/returnWorkOutMinisterProcess'
    }else if (data.flowId == 'ZjMeetingPlanRoom') {//中交计划会议流程
        url = 'http://weixin.fheb.cn:99/zjMeetingRoom/zjMeetingPlanRoomApplyProcess'
    } else if (data.flowId == 'stCcPurchase') {//世通重工出差
        url = 'zjFlowStEvectionProcess'
    }
    ...
    var options = {
        type: 2,
        title: title,
        content: url + ".html?workId=" + data.workId + "&trackId=" + data.trackId + "&nodeId=" + data.nodeId + "&flowId=" + data.flowId + "&title=" + data.title + "&code=" + code + "&nodeName=" + data.nodeName + "&status=" + data.status
    }
