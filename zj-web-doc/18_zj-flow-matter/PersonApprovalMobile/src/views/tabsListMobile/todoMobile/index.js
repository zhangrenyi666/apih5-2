...
class Index extends Component {
    constructor(props) {
        ...
        let apiName =
            page === 0 ? "getTodoList" : "getHasTodoList";
        this.state = {
            data: [],
            page: 1,
            limit: 10,
                                    ...
                                    <div style={{display:aa,display: 'flex',justifyContent: 'flex-end'}}>
                                        <Button type="primary" size="small" style={{display:aa}} onClick={(e) => {
                                            ...
                                            if(item.flowId === 'sanMinister'){
                                                dispatch(push(`${mainModule}MinisterPage/${item.workId}/${item.status}/${item.nodeName}/0`));
                                            }
                                            if(item.flowId === 'sanPerson'){
                                                dispatch(push(`${mainModule}PersonPage/${item.workId}/${item.status}/${item.nodeName}/0`));
                                            }
                                        }}>明细</Button>
