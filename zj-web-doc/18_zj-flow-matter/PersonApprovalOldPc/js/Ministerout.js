...
var table = $('#table').DataTable({
    ...
    "columnDefs": [//表格列配置
        ...
        },
        {
            ...
            "render": function (data) {
                var html = '<a style="color:blue;" href="./MinisterPerson.html?code=' + code + '&ministerApproveId=' + data.ministerApproveId + '&apih5FlowStatus='+data.apih5FlowStatus+'">人员明细</a>';
                return html;
            }
        }
    ]
        ...
        case "derive"://导出
            if (checkedData.length == 1) {
                layer.alert("确定导出此数据？", { icon: 0, }, function (index) {
                    window.location.href = 'http://192.168.1.223:89/ureport/excel?_u=file:sanZjFlowMinisterInfo.xml&url='+l.domain+'&delFlag=0&ministerApproveId=' + checkedData[0].ministerApproveId;
                    layer.close(index);
                }); 
            } else {
                layer.alert("只能选择一个",{ icon: 0,},function (index) {
