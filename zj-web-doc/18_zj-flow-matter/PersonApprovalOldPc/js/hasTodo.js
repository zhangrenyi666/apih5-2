...
$("body").on("click", "button", function () {
    ...
    switch (name) {
        ...
        case "process3":
            ...
            break;
        case "deriveMiniter"://部长导出
            var workIdListMiniter = '';
            var notExport = [];
            if (checkedData.length > 0) {
                for (var i = 0; i < checkedData.length; i++){
                    if (checkedData[i].flowId === 'sanMinister') {
                        if (i < checkedData.length-1) {
                            workIdListMiniter += '\''+checkedData[i].workId + '\',';
                        } else {
                            workIdListMiniter += '\''+checkedData[i].workId+'\''
                        }
                    } else {
                ...
                if (notExport.length > 0) {
                    layer.alert("请选择部长导出",{ icon: 0,},function (index) {
                        layer.close(index);
                        pager.page('remote')
                    });
                } else {
                    layer.alert("确定导出此数据？", { icon: 0, }, function (index) {
                        window.location.href = 'http://192.168.1.223:89/ureport/excel?_u=file:sanZjFlowMinisterInfo.xml&url='+l.domain+'&delFlag=0&workIdList=' + workIdListMiniter;
                        layer.close(index);
                        pager.page('remote')
                    });
                }
        ...
        case "derivePerson"://临聘人员导出
            ...
            if (checkedData.length > 0) {
                for (var i = 0; i < checkedData.length; i++){
                    if (checkedData[i].flowId === 'sanPerson') {
                        if (i < checkedData.length-1) {
...
function myOpen(title, data, url) {
    if(data.flowId == 'sanMinister'){//海外
      url = 'returnWorkOutMinisterProcess'
   }else if(data.flowId == 'sanPerson'){//海外
