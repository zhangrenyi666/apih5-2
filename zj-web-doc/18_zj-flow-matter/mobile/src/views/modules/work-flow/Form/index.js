...
import { Form, Input, Tabs, Timeline, Icon as IconByAntd, Spin, Modal, Button, Divider } from "antd";
...
class Home extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            //在线编辑文档的字段的布局
            docFormFormItemLayout: this.props.docFormFormItemLayout,
            ...
            pushNodeData: [],
            key: (flowId === 'ZjMeetingPlanSummary' || 'ZjMeetingPlanFallInSummary') ? '0' : '1'
        ...
        return {
            ...
            openDoc(data) {
                this.isInitEd(() => {
                    ...
                    //文件地址 | 文件名字 | 是否只读状态
                    childrenData.push(data.url, data.name, docIsReadOnly);
                    //父页面往子页面传值
                    //将token和用户信息传入文档窗口页面
                    window.ntkoSendDataToChildtext = function (ab) {
                        ntkoBrowser.ntkoSendDataToChild(url, childrenData);
                ...
                this.setState(
                    {
                        ...
                        docIsReadOnly: nodeVars ? !(nodeVars.ntkoEditFlag && nodeVars.ntkoEditFlag === '1') : false
                    },
                    () => {
    ...
    createOpenFlow = () => {
        ...
        this.qnnForm.getValues(true, (vals) => {
            // console.log(vals)
            //这个方法实际上只是为了验证下表单
            //因为传入的字段是对象格式  所以获取值时候 在表单块的值格式化会出 问题，所以使用下面 的validateFields即可
        });
        ...
        validateFields((err, values) => {
            ...
            for (let i = 0, j = formConfig.length; i < j; i++) {
                ...
                if (formConfig[i].type === "datetime" || formConfig[i].type === "date" || formConfig[i].type === "month") {
                    if (_apiBody[_field]) {
            ...
            //插入红头文件
            if (uploadRes) {
                _newApiBody[docFieldName] = uploadRes;
            }
    ...
    render() {
        ...
        //新增文档的按钮
        const addDocBtn = (<div style={{ display: docIsReadOnly ? "none" : "block" }}>
        ...
        const officeDom = (<div id="flow-doc" className={styles.flowDocContainer}>
            ...
            >
                ...
                {
                    uploadRes ? <div style={{ paddingTop: docIsReadOnly ? "0px" : '6px' }}>{
                    ...
                    >
                        ...
                        {workId === "add" ? (
                            <div className={styles.FlowConByAdd}>{FlowCon}</div>
                                ...
                                >
                                    {flowId === 'ZjMeetingPlanSummary' ? <TabPane tab={"详情信息"} key="0">
                                        <div style={{ height: document.documentElement.clientHeight - 90, position: 'relative' }}>
                                            <MyList
                                                myFetch={this.props.myFetch}
                                                upload={this.props.myUpload}
                                                fetchConfig={{
                                                    apiName: 'getZjMeetingPlanFallInListDeptId',
                                                    otherParams: {
                                                        workId: workId
                                                    }
                                                }}
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            ...
                                                            <div className={styles.top}>
                                                                <div className={styles.topl}>会议时间：{item.meetingTimeStr}</div>
                                                                <div className={styles.topr}>会议形式：{item.meetingForm}</div>
                                                            </div>
                                                            <Divider style={{ margin: "8px 0px" }} />
                                                            <div style={{ paddingLeft: '3%' }}>
                                                            ...
                                                            <Divider style={{ margin: "8px 0px" }} />
                                                            <div className={styles.top}>
                                                                <div className={styles.topl}>参会人数：{item.joinNumber}</div>
                                                                <div className={styles.topr}>预算费用(万元)：{item.budgetaryCost}</div>
                                                            </div>
                                                        </div>
                                                    );
                                                }}
                                    ...
                                    {flowId === 'ZjMeetingPlanFallInSummary' ? <TabPane tab={"详情信息"} key="0">
                                        <div style={{ height: document.documentElement.clientHeight - 90, position: 'relative' }}>
                                            <MyList
                                                myFetch={this.props.myFetch}
                                                upload={this.props.myUpload}
                                    ...
                                    <TabPane tab={flowId === 'ZjMeetingPlanSummary' ? '领导审批' : "详情"} key="1">
