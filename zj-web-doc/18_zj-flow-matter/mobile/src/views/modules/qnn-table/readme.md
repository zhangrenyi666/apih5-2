...
##### 详细配置项
    ...
    window.tableConfig.testPage = {
        fetchConfig: {//表格的ajax配置(更新后将自动刷新页面)  可为function 返回值为object
            apiName: 'getSxDocumentInfoList',//可为function 返回必须为string
            otherParams: {}, //需要固定写死的参数  可为function 返回必须为object
        },
        DrawerShow:false, //抽屉默认是否出现  默认false [bool]
        desc:'这是一段描述...',

        data: [], //表格数据（更新后将自动刷新页面） 默认为空 当fetchConfig存在时无意义
        ...
        //tabs配置存在的话打开右边抽屉将不使用formConfig配置 而是使用tabs中的配置
        tabs: [
            {
                //content就是qnn-form配置
                field:"one", //唯一标识名必传***
                ...
                content: { //name为qnnForm qnnTable的内容为qnnForm qnnTable的配置,自定义的话为reactDom
                    ...config
                }
            },
            {
            ...
            },
            {
                ...
                title: "表格",
                content: {
                    tabs:[], //在tabs项中需要打开qnn-form页面时需要重新配置tabs 否则将会无限循环的去使用父级的tabs配置
                    drawerConfig: {
                        width: "800px"
                        // maskClosable:false, //点击蒙层是否关闭抽屉  默认true
                    },
        ...
        formConfig: [
            {
                ...
                table: {
                    ...
                    */
                    ...
                    tdEdit: true, //单元格可编辑  目前支持的类型有 string、number、select、textarea
