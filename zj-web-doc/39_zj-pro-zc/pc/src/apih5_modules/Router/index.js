    ...
    if (isEn) {
        return (
            <div style={{
                ...
                <p>It will jump to the home page in {1} seconds（<a
                    onClick={() => {
                        tzFn(tzTimer);
                    }}
                >Manual jump to</a>）</p>
        ...
        return MyFetch(...arg).then(result => {
            if (result.code === "3003" || result.code === "3004") {
                ...
                if (androidApi) {
                    ...
                    Modal.alert(
                        (isEn ? "Identity information expiration prompt" : "身份信息过期提示"),
                        (isEn ? "Is the login information out of date?" : "登录信息过期是否重新登录？"),
                        [
