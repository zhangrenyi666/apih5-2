...
var supPlanExecutiveConditionId = l.getUrlParam("id") || "";
				       ...
				       callback: function(dataName, obj) {
				    	   ...
				    	   case "launch":
				    		   if (apih5FlowStatus=='2') {
				    			   layer.alert("已经提交了！不能重复提交", { icon: 0 }, function(index) {
				    				   layer.close(index);
				    				   $("#tab-system").Huitab({
				    					   index: 0
				    				   });
...
function loadPage() {
	if (yearSupPlanId) {
		l.ajax("getZjJdjhYearSupPlanDetails",{ yearSupPlanId: yearSupPlanId },function(data, message, success) {
			if (success) {
				...
				if (success) {
					...
					.prepend(
							$(
									...
									data.supDutyDepName +
									'" name="supDutyDepName" disabled></div> </div>'
							)
					);
					detailForm.setOpenData(data);
				} else {
