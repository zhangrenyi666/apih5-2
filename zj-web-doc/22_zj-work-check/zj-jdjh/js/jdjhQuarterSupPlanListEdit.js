...
var detailForm = $('#detailForm').detailLayer({
    ...
    onSave: function (obj, _params) {
        if (recordid) {
            _params.quarterSupPlanId = recordid
            l.ajax("updateZjJdjhQuarterSupPlan", _params, function (data, message, success) {
                if (success) {
            ...
            l.ajax("addZjJdjhQuarterSupPlan", _params, function (data, message, success) {
                if (success) {
                    recordid = data.quarterSupPlanId
                    layer.alert(message, { icon: 0, }, function (index) {
                        parent.pager.page('remote')
                        layer.close(index);
...
var detailLayer = $('#detailLayer').detailLayer({
    ...
    onSave: function (obj, _params) {
        l.ajax("updateZjJdjhQuarterSupMatter", _params, function (data, message, success) {
            if (success) {
                pager.page('remote', { quarterSupPlanId: recordid })
                layer.alert(message, { icon: 0, }, function (index) {
                    layer.close(index);
                    obj.close()
                });
    ...
    onAdd: function (obj, _params) {
        l.ajax("addZjJdjhQuarterSupMatter", _params, function (data, message, success) {
            if (success) {
                pager.page('remote', { quarterSupPlanId: recordid })
                layer.alert(message, { icon: 0, }, function (index) {
                    layer.close(index);
...
$("body").on("click", "button", function () {
    ...
    switch (name) {
        case "add":
            if (recordid) {
                detailLayer.open({ isAdd: true, quarterSupPlanId: recordid })
            } else {
        ...
        case "del":
            if (checkedData.length == 0) {
                ...
                layer.confirm("确定删除？", { icon: 0, }, function (index) {
                    l.ajax("batchDeleteUpdateZjJdjhQuarterSupMatter", checkedData, function (data, success, message) {
                        if (success) {
                            pager.page('remote')
                        }
                    })
...
function myOpen1(index, type) {
    ...
    switch (type) {
        case 'xq':
            params = {};
            params.quarterSupMatterId = rowData.quarterSupMatterId;
            //console.log('params:',params)
            l.ajax('getZjJdjhQuarterSupMatterDetail', params, function (res) {
