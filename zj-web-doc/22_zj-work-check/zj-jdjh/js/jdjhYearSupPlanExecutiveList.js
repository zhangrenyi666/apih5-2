    ...
    "columnDefs": [//表格列配置
        ...
        },
        {
            ...
            "render": function (data, index, row) {
                var a;
                if (data) {
                    a = '<a style="color:blue;"  onclick="myOpen(\'季度监督执行情况确认\',\'' + data.yearSupPlanId + '\',\'' + 'jdjhYearSupPlanExecutiveListEdit' + '\')" href="javascript:void(0);">' + data.supObj + '</a>'
                }
                return a;
            }
        },     
...
$("body").on("click", "button", function () {
    ...
    switch (name) {
        case "add":
            myOpen('问题清单确认--新增', "", 'jdjhYearSupPlanExecutiveListEdit	')
            break;
        case "edit":
            if (checkedData.length == 1) {
                var recordid = checkedData[0].checkIssueId
                if (checkedData[0].apih5FlowStatus === "0") {
                    myOpen('问题清单确认--编辑', recordid, 'jdjhYearSupPlanExecutiveListEdit	')
                } else {
                        ...
                        l.ajax("reviseTheScheme", { recordid: recordid }, function (data, message, success) {
                            if (success) {
                                ...
                                layer.alert(message, { icon: 0 }, function (index2) {
                                    layer.close(index2);
                                    myOpen('方案等级确认--编辑', recordid, 'jdjhYearSupPlanExecutiveListEdit	')
                                });
                            }
                        })
                        layer.close(index);
