    ...
    "columnDefs": [//表格列配置
        ...
        },
        {
            ...
            "render": function (data, index, row) {
                var a;
                if (data) {
                    a = '<a style="color:blue;"  onclick="myOpen(\'季度监督确认\',\'' + data.quarterSupPlanId + '\',\'' + 'jdjhQuarterSupPlanListDetail' + '\')" href="javascript:void(0);">' + data.supObj + '</a>'
                }
                return a;
            }
        },     
...
$("body").on("click", "button", function () {
    ...
    switch (name) {
        case "add":
            myOpen('季度监督确认--新增', "", 'jdjhQuarterSupPlanListEdit')
            break;
        case "edit":
            if (checkedData.length == 1) {
                var recordid = checkedData[0].quarterSupPlanId
                if (checkedData[0].apih5FlowStatus === "0") {
                    myOpen('季度监督确认--编辑', recordid, 'jdjhQuarterSupPlanListEdit')
                } else {
            ...
            if (checkedData.length == 0) {
                ...
                if (can) {
                    layer.confirm("确定删除？", { icon: 0, }, function (index) {
                        l.ajax("batchDeleteUpdateZjJdjhQuarterSupPlan", checkedData, function (data, message, success) {
                            if (success) {
                                pager.page('remote')
