...
export default class Inspect extends Component {
    ...
    loadTherr() {
        let body = {
            ...
            yearSupPlanId: this.props.yearSupPlanId
        }
        this.props.myFetch("getZjJdjhYearSupMatterList", body).then(({ success, message, data }) => {
            if (success) {
                this.setState({
                    data: data,
                    value: ""
                    ...
                    <PullToRefresh
                        ...
                        onRefresh={() => {
                            this.setState({
                                ...
                                myFetch("getZjJdjhYearSupMatterList", body).then(({ success, message, data }) => {
                                    if (success) {
                                        this.setState({
                                    ...
                                    return (
                                        <div className={s.centers} key={index} style={{ borderLeft: '5px solid red' }}>
                                            <div className={s.head}>
                                                <div className={s.headl}><span style={{fontWeight:'bold'}}>部门：</span>{item.coopDept}</div>
