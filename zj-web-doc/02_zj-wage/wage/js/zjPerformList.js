...
var pager = $("#pager").page({
    remote: {//ajax请求配置
        url: l.getApiUrl("getZjPerformList"),
        params: {
        },
...
var leading = $('#leading').detailLayer({
    ...
    onAdd: function (obj, _params){
        ...
        l.ajax('batchInputPerform', {"importFileList":_params.importFileList,"year":_params.year,"month":_params.month}, function  (data,message,success) {		
...
var send = $('#send').detailLayer({
    ...
    onAdd: function (obj, _params){
	    l.ajax('getSendPerformNum', {"year":_params.year,"month":_params.month}, function (data,message,success) {
			//console.log(data)			
			if(data=="0"){
				layer.alert("该月份未导入任何工资数据或本月工资已发送！")				
			}else{				
			  layer.confirm("发现"+data+"人的工资条，确定发送？", { icon: 0,}, function (index) {
              l.ajax('sendPerform', {"year":_params.year,"month":_params.month}, function (data,message,success) {
				  if(success){
                     if(data=="0"){
