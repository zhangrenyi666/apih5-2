...
export default class PullPersion extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      neetNumber: this.props.neetNumber //需要数量
    };
    ...
    this.textObj = {
      ...
      rightTitle: this.props.textObj
        ? this.props.textObj.rightTitle
        : "已选择的部门或成员",
      ...
      minNumber: this.props.textObj
        ...
        : `选择数量不足，至少需要选择${this.state.minNumber}`
    };
  ...
  componentDidMount() {
    if (this.fetchConfig.apiName) {
      ...
      this.myFetch(this.fetchConfig.apiName, this.fetchConfig.params).then(
  ...
  nodeClick(select, nodeInfo) {
    ...
    let { ModalData, selectType, maxNumber, neetNumber } = this.state;
    let {
      node: {
    ...
    let { residueInventory } = _o;
    if (!residueInventory && neetNumber) {
      message.error("抱歉，库存不足了哟！去增加库存或者选择其他书本吧。");
      return;
    }
    let obj = {
      ..._o,
  ...
  renderTreeNodes(data = []) {
    ...
    if (data) {
      return data.map(item => {
        ...
        //剩余库存（门和窗项目定制版）
        let residueInventory = "";
        if (item.residueInventory || item.residueInventory === 0) {
          residueInventory = (
            <span style={{ fontSize: "12px", color: "#666" }}>
              （在库
              <span style={{ color: item.residueInventory ? "" : "red" }}>
                {item.residueInventory || 0}
              </span>
              本）
            </span>
          );
        ...
        if (item.search) {
          ...
          return (
            <TreeNode
              className={`${item[type] === "2" ? "perNode" : ""} myNode  ${
                selectType === "0"
                  ? "blueImg"
                  : selectType === "2"
                    ? "blueImg"
                  ...
                  <Col key={i} span={6} md={6} sm={8} xs={12} xxl={4}>
                    <div
                      className={`${styles.colCon} ${
                        item[type] === "2" ? styles.persion : styles.folder
                      }`}
                    >
                      <Tooltip
                        title={
                          <span>
                            {item[label]}
                            {neetNumber ? <span> *{item.number}</span> : null}
                          </span>
                        }
                      >
                        <span>
          ...
          >
            {loading ? (
              <div className="example">
                <Spin />
                <center style={{ color: "#999", fontSize: "12px" }}>
              ...
              <div className="modalBody">
                <div className={styles.treeContainer}>
                  ...
                  {this.search || search ? (
                    <Search
                      ...
                      placeholder={
                        this.searchPlaceholder || "请输入搜索条件..."
                      }
                ...
                <div className={styles.selectedContainer}>
                  <span className={styles.selectedContainerTitle}>
