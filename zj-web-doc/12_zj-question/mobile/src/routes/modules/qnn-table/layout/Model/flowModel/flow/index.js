...
import { Form, Button, Timeline, Icon } from "antd";
...
class FlowForm extends Component {
  render() {
    ...
    // console.log("flowFormColumn ：", flowFormColumn);
    ...
    case "com.horizon.wf.action.ActionSave":
      alert("暂存", "", [
        { text: "取消", onPress: () => console.log("cancel") },
        {
          text: "确定",
          onPress: () => {
            actionFlow.bind(this)({ operateObj, visible: false, close: true });
          }
        }
      ]);
      break;
          ...
          {curNextNodes.map((item, index) => {
            ...
            return (
                        ...
                        let actionNextNodes = curNextNodes.map(item3 => {
                          if (item3.selectId === selectId) {
                            if (nodeGroup) {
                              ...
                              item3.isSelect =
                                item3.isSelect === "true" ? "false" : "true";
                            }
                          } else {
                            if (nodeGroup) {
              ...
              >
                <span>
                  {isDone === "true" ? (
                    <span style={{ color: "#108ee9" }}>{"[已]"}</span>
                  ) : (
                    ""
                  )}
                  {selectName}
                                  ...
                                  for (const key in authors) {
                                    item3.authors[key] = {
                                      ...
                                      selectAuthorNewApih5:
                                        value[key].selectAuthorNewApih5
                                    };
                                  }
                                }
                                return item3;
              ...
              >
                {isMobile()
                  ? getFieldDecorator(authorId + "[selectAuthorNewApih5]", {
                      ...
                      initialValue:
                        value[authorId]["selectAuthorNewApih5"] || []
                    })(
                      <PullPersionMobile
                        search={value[authorId].initAuthorApih5SearchApi}
                        searchParamsKey={"search"}
                        searchApi={
                          value[authorId].initAuthorApih5SearchApi
                            ? value[authorId].initAuthorApih5SearchApi
