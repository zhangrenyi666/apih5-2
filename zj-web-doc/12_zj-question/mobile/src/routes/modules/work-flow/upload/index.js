...
class Upload extends Component {
    ...
    state = {
        value: this.props.defaultValue || [], //默认值 
        prewShow: false,
    ...
    fileSelected = () => {
        const _this = this;
        const apiName = this.fetchConfig.apiName || 'upload';
        ...
        $.ajax({
            url: `${domain.api}${apiName}`,
            type: 'post',
            dataType: 'json',
            data: _formData,
            beforeSend: function (xhr) {
                xhr.setRequestHeader("token", _this.props.token);
            },
            xhr: function () {
            ...
            success: function ({ message, success, data }) {
                if (success && data) {
                    ...
                    _data['fileUrl'] = data['url']
                    _data['realUrl'] = data['url']
                    _data['thumbPath'] = data['url']
                    value.push(_data);
                    // value.push(data);
                    _this.setState({
                        value
    ...
    render() {
        ...
        return (
            <div className={s.root} >
                ...
                <div className={s.filesCon}>
                    {
                        value && value.map((item, index) => {
                            let { url, name, mobileUrl, fileUrl, fileName } = item;
                            url = url || fileUrl;
                ...
                {/* 表单区域 */}
                <form className={s.formCon} id="uploadForm" encType="multipart/form-data" method="post" action="XXXXXX">
                    <input type="file" hidden name="files" id={fieldName} onChange={this.fileSelected} />
