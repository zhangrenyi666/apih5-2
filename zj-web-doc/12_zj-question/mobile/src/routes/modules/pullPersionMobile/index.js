...
class PullPersionMobile extends Component {
    ...
    textObj = {
        //搜索框的占位符
        searchPlaecholder: this.props.searchPlaecholder || '搜索',
        //加载中的文字
        ...
        //添加按钮的文字
        addBtn: this.props.textObj ? this.props.textObj.addBtn : '选择项目',
        //当selectType===1时弹出的提示
    ...
    componentDidMount() {
        ...
        if (fetchConfig.apiName) {
            ...
            this.myFetch(fetchConfig.apiName, fetchConfig.params)
    ...
    render() {
            ...
            this.defaultValue ?
                this.defaultValue.map((item, index) => {
                    ...
                    return <div key={index} style={{ paddingRight: edit ? '25px' : '5px' }} className={`${s.fItem}  ${_type === '2' ? s.per : s.folder}`}>
                        {_label}
                        <span style={{ display: !edit ? 'none' : '' }} onClick={() => { 
                            this.delFn(_value)
                            this.props.updateParent('');
                             }} className={s.fdel}>×</span>
                    </div>
                })
                :
                null
        ...
        //渲染导航条
        const renderNav = (<div className={'pull-person-mobile-navContainer'}>
            {
        ...
        const sidebar = (<div className={s.pullPersonContainer}>
            ...
            {/* 底部信息框 */}
            <div className={'pull-person-mobile-bottomInfo'} style={{
