...
var pager = $("#pager").page({
    remote: {// ajax请求配置
        url: l.getApiUrl("getZjXmQuestionVisitMemberList"),
        params: {
        },
...
var detailLayer = $('#detailLayer').detailLayer({
    ...
    onSave: function (obj, _params) {
        l.ajax('updateZjXmQuestionSetupMember', _params, function (data) {
            pager.page('remote')
            obj.close()
        })
    },
    onAdd: function (obj, _params) {
        l.ajax('saveZjXmQuestionVisitMember', _params, function (data) {
            pager.page('remote')
            obj.close()
        })
    }
        ...
        case "del":
            if (checkedData.length == 0) {
                ...
                layer.confirm("确定删除？", { icon: 0, }, function (index) {
                    l.ajax("batchDeleteUpdateZjXmQuestionVisitMember", checkedData, function () {
                        pager.page('remote')
                    })
                    layer.close(index);
                });
