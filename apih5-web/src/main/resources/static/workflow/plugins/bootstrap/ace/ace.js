		...
		this.getUrl = function(url, hash, manual_trigger) {
			...
			.done(function(result) {
				...
				var callback = function(result) {
                    result = result.replace(/<(title|link)([\s\>])/gi,'<div class="hidden ajax-append-$1"$2')
				...
				/**
				 * 增加格式化result的扩展方法
				 * @author lichao 2017-4-11
				 */
                if(options.format_result && typeof options.format_result === 'function') {
                    options.format_result.apply(this, [result, callback]);
                }else {
                    callback(result);
                }
			})
	...
	var Widget_Box = function(box, options) {
		...
		this.fullscreen = function() {
			...
			if(!this.$box.hasClass('fullscreen')) {
				...
				//applyScrollbars(this.$box, true); update by zhouwf
			}
			else {
				$icon.addClass($icon_expand).removeClass($icon_compress);
				this.$box.removeClass('fullscreen');
				
				//applyScrollbars(this.$box, false); update by zhouwf
			}
